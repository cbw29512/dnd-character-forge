...
The generated records intentionally contain stat-line metadata plus conservative
source pointers. They do not infer attack/save resolution or rewrite spell rules.
That keeps the 2014 quick-reference path fail-closed while still giving every
Forge-supported SRD spell a verified source/page/stat-line record.
"""
...
SPELL_HEADER_RE = re.compile(
    rf"^(?:(1st|2nd|3rd|[4-9]th)[^\n]*?level\s+({SCHOOL_RE})|({SCHOOL_RE})\s+cantrip)(?:\s+\(ritual\))?\s*$",
    re.IGNORECASE | re.MULTILINE,
)
SOURCE_URL = "https://media.wizards.com/2023/downloads/dnd/SRD_CC_v5.1.pdf"
EXPECTED_PDF_PAGES = 403
EXPECTED_COUNT = 319
SPELL_PAGE_START = 113  # zero-based PDF page index; printed SRD page 114
SPELL_PAGE_END = 194    # exclusive; printed page 194 finishes Zone of Truth
...
def parse_fields(block: str, title: str, page: int) -> tuple[str, str, str, str, str]:
    try:
        ...
        if not duration_match:
            raise RuntimeError(f"{title} (SRD p. {page}): missing Duration line")
        description = clean(block[duration_match.end():])
        if not description:
            raise RuntimeError(f"{title} (SRD p. {page}): missing description body")
        return values["Casting Time"], values["Range"], values["Components"], values["Duration"], description
    except Exception as error:
        if isinstance(error, RuntimeError):
            raise
    ...
    try:
        ...
        for index, match in enumerate(matches):
            ...
            if not title or ":" in title:
                raise RuntimeError(f"invalid spell title before header {match.group(0)!r}: {title!r}")
            if index + 1 < len(matches):
                next_pre = text[:matches[index + 1].start()].rstrip()
                block_end = next_pre.rfind("\n") + 1
            else:
                block_end = len(text)
            page_pos = bisect.bisect_right(page_starts, name_start) - 1
            page = page_numbers[max(page_pos, 0)]
            block = text[name_start:block_end].strip()
            ordinal = match.group(1)
            level = LEVELS[ordinal.lower()] if ordinal else 0
            school = clean(match.group(2) or match.group(3)).title()
            casting_time, range_text, components, duration, description = parse_fields(block, title, page)
            spell_id = slug(title)
            if spell_id in seen:
                raise RuntimeError(f"duplicate spell id parsed: {spell_id}")
            seen.add(spell_id)
            ritual = "(ritual)" in clean(match.group(0)).lower()
            concentration = duration.lower().startswith("concentration")
            has_upcast = "at higher levels" in description.lower()
            records.append({
                "id": spell_id,
                "name": title,
                "level": level,
                ...
                "duration": duration,
                "resolution": f"See SRD 5.1 page {page} for spell resolution.",
                "effect": f"Official SRD 5.1 spell effect; see page {page} for the complete rules text.",
                "concentration": concentration,
                "ritual": ritual,
                "upcast": f"See SRD 5.1 page {page} for higher-level spell-slot scaling." if has_upcast else None,
                "source": "SRD 5.1",
                "srdPage": page,
            })
...
def render(records: list[dict], digest: str, source_bytes: int) -> str:
    try:
        ...
        return (
            ...
            f"export const SPELL_REFERENCE_2014_GENERATED=Object.freeze({payload});\n"
        )
    except Exception as error:
...
def main() -> None:
    try:
        if len(sys.argv) != 3:
            raise RuntimeError("usage: generate-srd-51-spell-reference.py <SRD PDF> <output JS>")
        pdf_path = Path(sys.argv[1])
        output_path = Path(sys.argv[2])
        records, digest, source_bytes = parse(pdf_path)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text(render(records, digest, source_bytes), encoding="utf-8")
        print(f"source sha256={digest} bytes={source_bytes}")
        print(f"generated {len(records)} SRD 5.1 spell references -> {output_path}")
    except Exception as error:
        print(f"[generate-srd-51] failed: {error}", file=sys.stderr)
