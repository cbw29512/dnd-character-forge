...
EXPECTED_SOURCE_SHA256="2504d2a0abb0a4d491a939be4f17910a2dde0312570ab8d208080225ccf0a1f0"
EXPECTED_SOURCE_BYTES=3158713
...
def verify_source(pdf_path:Path)->tuple[str,int]:
    try:
        ...
        if digest!=EXPECTED_SOURCE_SHA256:
            raise RuntimeError(f"SRD 5.1 source SHA-256 {digest} does not match pinned {EXPECTED_SOURCE_SHA256}.")
        if len(payload)!=EXPECTED_SOURCE_BYTES:
            raise RuntimeError(f"SRD 5.1 source size {len(payload)} does not match pinned {EXPECTED_SOURCE_BYTES} bytes.")
