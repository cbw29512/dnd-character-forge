...
function enforceRawState(){
  try{
    state.sourceMode=SOURCE.RAW;
    state.homebrew=[];
  }catch(error){console.error("[app] RAW state enforcement failed",error);throw error;}
}
...
function bindConstraints(){
  try{
    ...
    document.getElementById("class").addEventListener("change",()=>{state.constraints.subclass="random";state.spellSelections=emptySpells();populateSubclasses(state);refreshSpellPicker(state);});
    document.getElementById("level").addEventListener("change",()=>{state.spellSelections=emptySpells();populateSubclasses(state);refreshSpellPicker(state);});
    document.getElementById("ruleset").addEventListener("change",event=>{state.ruleset=event.target.value;state.constraints.species="random";state.constraints.class="random";state.constraints.subclass="random";state.constraints.background="random";state.spellSelections=emptySpells();enforceRawState();populateOptions(state);refreshSpellPicker(state);});
...
function showTab(name){
  try{
    if(!["forge","pregens"].includes(name))throw new Error("Production Character Forge exposes RAW Forge and Pregens only.");
  ...
  try{
    document.getElementById("result").addEventListener("click",async event=>{
      try{
        ...
        if(action==="save"){
          if(state.currentCharacter?.sourceMode!==SOURCE.RAW||state.currentCharacter?.homebrew?.length)throw new Error("Only validated RAW characters can be saved.");
          const entry=await savePregen(state.currentCharacter);renderPregenLibrary();showToast(`${entry.name} saved to My Pregens.`);
        }
...
function viewSavedPregen(entry){
  try{
    ...
    if(character.sourceMode!==SOURCE.RAW||character.homebrew?.length)throw new Error("Production Character Forge opens RAW pregens only.");
    ...
    state.spellSelections=character.spells?{cantrips:[...character.spells.cantrips.all],spellbook:[...(character.spells.spellbook?.all||[])],prepared:[...character.spells.prepared.all]}:emptySpells();
    state.homebrew=[];
    syncControls();
...
function forge(){
  ...
  try{
    ...
    if(state.currentCharacter.sourceMode!==SOURCE.RAW||state.currentCharacter.homebrew?.length)throw new Error("RAW validation rejected the generated character.");
    renderCharacter(state.currentCharacter,document.getElementById("result"));
  }catch(error){showError(error);}finally{window.setTimeout(()=>button.classList.remove("is-forging"),160);}
}
