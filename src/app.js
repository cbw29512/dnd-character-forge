...
import { bindSpellPicker } from "./ui/spell-picker.js";
...
function bindMode(){
  try{document.getElementById("sourceMode").addEventListener("change",event=>{state.sourceMode=event.target.value;document.getElementById("homebrewPanel").hidden=state.sourceMode!==SOURCE.HOMEBREW;});}
...
function forge(){
  ...
  try{button.classList.add("is-forging");document.getElementById("error").hidden=true;state.currentCharacter=generateCharacter(state);renderCharacter(state.currentCharacter,document.getElementById("result"));}
...
function showToast(message,isError=false){
  try{const toast=document.getElementById("toast");toast.textContent=message;toast.classList.toggle("is-error",isError);toast.hidden=false;window.clearTimeout(showToast.timer);showToast.timer=window.setTimeout(()=>{toast.hidden=true;},3600);}
