...
import { composedDossierPortraitArt } from "./dossier-art-layer-renderer.js";
...
// Exact curated art always outranks reusable composition and class fallback.
...
export function dossierFallbackArt(classId,artDirection={}){
  try{
    ...
    if(entry)return renderCuratedPortrait(classId,variantKey,entry);
    const composed=composedDossierPortraitArt(classId,artDirection);
    return composed||classPlaceholderArt(classId);
  }catch(error){
...
function renderCuratedPortrait(classId,variantKey,entry){
