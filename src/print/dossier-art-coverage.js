...
import { curatedDossierPortraitIds } from "./dossier-portrait-assets.js";
...
export function dossierArtCoverage(){
  try{
    ...
    const exactIds=curatedDossierPortraitIds(),exact=exactIds.filter(id=>supported.has(id)),orphaned=exactIds.filter(id=>!supported.has(id));
...
function addVariant(map,{ruleset,backgroundId,classId,pathId,kind}){
  ...
  if(existing&&existing.classId!==classId)throw new Error(`Variant key collision: ${variantKey} maps to ${existing.classId} and ${classId}.`);
  if(existing){
