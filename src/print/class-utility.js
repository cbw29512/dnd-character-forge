...
function monkUtility(character){
  try{
    const m=character.monk;if(!m)return null;const resource=character.level>=2?m.resourcePoints:"—",resourceUnit=character.level>=2?`${m.resourceName} Points`:"unavailable",capstone=m.quiveringPalm?(character.ruleset==="2014"?"Quivering Palm 3 Ki: fail 0 HP / success 10d10 Necrotic":"Quivering Palm 4 Focus: 10d12 Force / half on save"):null;
    return{title:"Centered Discipline",kind:"monk",stats:[stat(m.resourceName,resource,resourceUnit),stat("Martial Arts",m.martialArtsDie,"damage die"),stat("Movement",m.unarmoredMovementBonus?`+${m.unarmoredMovementBonus}`:"—","ft unarmored"),stat("Save DC",monkSaveDc(character),"Monk features")],note:character.ruleset==="2014"?`${m.allSaveProficiency?"Diamond Soul · ":""}${capstone|| (m.flurryOfBlows?"Flurry / Patient Defense / Step of the Wind":"Martial Arts ready")}${character.level>=20?" · Perfect Self":""}`:`${m.allSaveProficiency?"Disciplined Survivor · ":""}${capstone||(m.heightenedFocus?"Heightened Focus":"Focus techniques ready")}${m.bodyAndMind?" · Body and Mind":""}`};
