...
export function buildNarrativeDossier(character,{quickTurn=[]}={}){
  try{
    ...
    return Object.freeze({
      ...
      roleplay:Object.freeze({quote:classStory.quote,guidance:`Play ${character.name} as a ${storyArc.label.toLowerCase()} whose competence matters more than theatrics. Let the ${className} identity shape how problems are approached, while the ${background} background explains which obligations still feel personal. Observe first, speak with purpose, and let emotion show through choices rather than speeches.`})
    });
