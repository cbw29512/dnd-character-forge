...
function extend(raw,bardClass,bardSubclass,monkClass,monkSubclass,weaponAdditions){
  try{
    for(const classId of ["bard","monk"]){
      if(raw.classes.some(item=>item.id===classId)||raw.subclasses.some(item=>item.classId===classId))throw new Error(`Base ${raw.ruleset} RAW catalog already contains ${classId}; remove the extension instead of duplicating it.`);
    }
    ...
    return Object.freeze({...raw,classes:Object.freeze([...raw.classes,bardClass,monkClass]),subclasses:Object.freeze([...raw.subclasses,bardSubclass,monkSubclass]),weapons:Object.freeze({...raw.weapons,...weaponAdditions})});
  }catch(error){console.error(`[forge-data] ${raw?.ruleset||"unknown"} extension failed`,error);throw error;}
}

export const FORGE_2014=extend(RAW_2014,BARD_CLASS_2014,BARD_SUBCLASS_2014,MONK_CLASS_2014,MONK_SUBCLASS_2014,MONK_WEAPONS_2014);
export const FORGE_2024=extend(RAW_2024,BARD_CLASS_2024,BARD_SUBCLASS_2024,MONK_CLASS_2024,MONK_SUBCLASS_2024,MONK_WEAPONS_2024);
