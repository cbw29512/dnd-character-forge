...
function buildBarbarianAudit(character,validation){
  try{
    ...
    const source=rulesetSource(character.ruleset),classSource=barbarianEntityProvenance(character.ruleset,"class"),homebrewCount=character.homebrew?.length||0,forgeOriginal=isBarbarianForgeOriginal(character.subclass),rawIntegrity=character.sourceMode===SOURCE.RAW&&homebrewCount===0&&!forgeOriginal;
    ...
    if(character.subclass)mechanics.push(mechanic("Subclass",character.subclass.name,barbarianSubclassProvenance(character)));mechanics.push(mechanic("Level",String(character.level),classSource));
    const contentCheck=forgeOriginal?`${character.subclass.name} is explicitly labeled Character Forge Original game content; it is not represented as official D&D RAW or SRD content.`:"Displayed Barbarian rules and identity carry verified SRD source locators.";
    return auditEnvelope(character,source,rawIntegrity,homebrewCount,mechanics,["Character generation completed with zero validation errors.",barbarianEditionCheck(character.ruleset),contentCheck],{forgeOriginal});
...
function auditEnvelope(character,source,rawIntegrity,homebrewCount,mechanics,checks,options={}){try{const forgeOriginal=Boolean(options.forgeOriginal),license=forgeOriginal?"CC BY 4.0 SRD foundation + Character Forge Original game content":LICENSE,scope=forgeOriginal?"SRD class mechanics plus explicitly labeled Character Forge Original subclass content; official non-SRD D&D subclasses are not reproduced.":"Character Forge verified SRD coverage only; unsupported content is unavailable instead of guessed.",integrityCheck=forgeOriginal?"Content integrity passed: SRD mechanics remain sourced and the selected Forge Original subclass is clearly separated from official RAW.":character.sourceMode===SOURCE.RAW?"RAW integrity passed: no Homebrew mechanics are present in this character.":`Homebrew mode is explicit: ${homebrewCount} structured Homebrew entr${homebrewCount===1?"y":"ies"} applied on top of RAW.`;return{status:"PASS",sourceMode:character.sourceMode,rawIntegrity,ruleset:character.ruleset,rulesLabel:RULESET_LABELS[character.ruleset],sourceDocument:source.document,sourceVersion:source.version,sourceUrl:SRD_LANDING_URL,sourcePdfUrl:source.pdfUrl,license,scope,mechanics,checks:[...checks,integrityCheck]};}catch(error){console.error("[audit-router] audit envelope failed",error);throw error;}}
