...
function buildPaladinAudit(character,validation){
  try{
    ...
    if(character.subclass)mechanics.push(mechanic("Subclass",character.subclass.name,paladinEntityProvenance(character.ruleset,"subclass")));
...
function auditEnvelope(character,source,rawIntegrity,homebrewCount,mechanics,checks){
  ...
  catch(error){console.error("[audit-router] audit envelope failed",error);throw error;}
}
