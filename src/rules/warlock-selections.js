...
import { resolveInvocationCantripTargets, warlockInvocationById, warlockInvocationsFor } from "../data/warlock-invocations.js";
...
export function resolveWarlockSelections({ruleset,level,subclassId=null,selections={}}){
  try{
    const rawFixed=selections.eldritchInvocations||[],fixed=rawFixed.filter(Boolean),progression=warlockProgressionFor(ruleset,level,subclassId);
    ...
    const targetPreferences=targetPreferencesForResolvedInvocations(all,rawFixed,selections.warlockInvocationTargets||[]),invocationCantripTargets=resolveInvocationCantripTargets(ruleset,all,targetPreferences);
    ...
    return Object.freeze({pactBoon,familiarForm,bonusSkills,invocationCantripTargets,invocations:Object.freeze({selected:Object.freeze([...fixed]),randomized:Object.freeze(randomizedInvocations(all,fixed)),all:Object.freeze(all)})});
...
function targetPreferencesForResolvedInvocations(all,rawFixed,rawTargets){
  try{
    const queues=new Map();
    rawFixed.forEach((id,index)=>{if(!id)return;if(!queues.has(id))queues.set(id,[]);queues.get(id).push(rawTargets[index]||null);});
    return all.map(id=>{const queue=queues.get(id);return queue?.length?queue.shift():null;});
  }catch(error){console.error("[warlock-selections] target preference alignment failed",error);throw error;}
}
