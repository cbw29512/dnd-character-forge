...
export function resolveWarlockSelections({ruleset,level,subclassId=null,selections={}}){
  try{
    ...
    while(all.length<progression.invocations){const legal=catalog.filter(option=>!all.includes(option.id)&&option.minLevel<=Number(level)&&legacyPactLegal(ruleset,option,pactBoon)&&revisedDependencyLegal(ruleset,option,all));if(!legal.length)throw new Error(`No verified legal Eldritch Invocation can fill choice ${all.length+1} of ${progression.invocations}.`);all.push(pick(legal).id);}
    ...
    return Object.freeze({pactBoon,familiarForm,invocations:Object.freeze({selected:Object.freeze([...fixed]),randomized:Object.freeze(all.filter(id=>!fixed.includes(id))),all:Object.freeze(all)})});
