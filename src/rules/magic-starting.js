...
function candidatesFor(classId,rarity){
  try{return ITEM_CATALOG.filter(item=>item.rarity===rarity&&(!item.kind||item.kind!=="weapon"||classId!=="wizard"));}
...
export function generateStartingMagic({ruleset,level,mode,classId}){
  try{
    const plan=startingMagicPlan(ruleset,level,mode);
    let index=0;
    for(const [rarity,count] of Object.entries(plan.allowance||{}))for(let i=0;i<count;i++){
      const candidates=candidatesFor(classId,rarity),item=pick(candidates,index++);
      if(!item)throw new Error(`No verified ${rarity} magic item is available for ${classId}.`);
      plan.items.push({...item,source:plan.source});
    }
    return plan;
