...
export function validate2014Species(character){
  try{
    ...
    const monkSpeed=character.class?.id==="monk"&&!character.equipment?.armor&&!character.equipment?.shield?(character.monk?.unarmoredMovementBonus||0):0,advancementSpeed=Number(character.advancementSpeedBonus||0),expectedSpeed=species.speed+(character.barbarian?.speedBonus||0)+monkSpeed+advancementSpeed;
    if(character.size!==species.size)errors.push(`${species.name} size should be ${species.size}.`);if(character.speed!==expectedSpeed)errors.push(`${species.name} final speed should be ${expectedSpeed} ft after legal movement modifiers.`);
