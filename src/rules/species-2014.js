...
export function validate2014Species(character){
  try{
    ...
    const expectedSpeed=species.speed+(character.class?.id==="barbarian"&&character.level>=5&&!character.equipment?.armor?10:0);if(character.size!==species.size)errors.push(`${species.name} size should be ${species.size}.`);if(character.speed!==expectedSpeed)errors.push(`${species.name} final speed should be ${expectedSpeed} ft.`);
