...
const NORMAL_LEVELS=Object.freeze([4,8,12,16]);
const LEVELS_2014=Object.freeze([4,8,12,16,19]);
const EPIC_BOONS=Object.freeze([
  "boon-combat-prowess","boon-dimensional-travel","boon-fate",
  "boon-irresistible-offense","boon-night-spirit","boon-truesight"
]);

export function applyBarbarianAdvancement(character,data){
  try{
    if(character.class.id!=="barbarian")return character;
    const next=structuredClone(character),choices=[];
    if(character.ruleset==="2014"){
      let grapplerTaken=false;
