...
export function validateCharacter(character,mode){
  try{
    ...
    if(character.class.id==="barbarian")validateBarbarian(errors,character);if(character.class.spellcasting==="wizard")validateWizard(errors,character);if(character.class.spellcasting==="cleric")validateCleric(errors,character);if(!Number.isInteger(character.ac)||character.ac<1)errors.push("Armor Class failed calculation.");if(!Number.isInteger(character.hp)||character.hp<1)errors.push("Hit Points failed calculation.");return{valid:errors.length===0,errors};
  }catch(error){console.error("[validation] character validation failed",error);throw error;}
}
function validateBarbarian(errors,character){
  try{
    const row=barbarianProgression(character.ruleset,character.level),resources=Object.fromEntries((character.classResources||[]).map(item=>[item.id,item.value]));
