...
export function buildForgeOriginalAudit(character,validation){
  try{
    ...
    const originalBackground=isForgeOriginalBackground(character?.background),originalSubclass=isForgeOriginalSubclass(character?.subclass)||isBarbarianForgeOriginal(character?.subclass);
    ...
    if(originalSubclass){
      const records=originalSubclassFeatureRecordsFor(character.ruleset,character.class.id,character.level,character.subclass.id);
      ...
      checks.push(`${records.length} active original subclass feature${records.length===1?"":"s"} passed the encoded level-gate progression for level ${character.level}.`);
    }
    ...
    return{
      ...
      scope:"SRD base mechanics plus explicitly labeled Character Forge Original content; official non-SRD D&D material is not reproduced.",mechanics,checks
    };
