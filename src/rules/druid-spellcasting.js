...
export function validateDruidSpellSelections(character,selections={}){
  try{
    ...
    const redundant=prepared.filter(id=>always.has(id));if(redundant.length)throw new Error(`${redundant.join(", ")} is already always prepared and cannot consume normal Druid preparation.`);
...
export function buildDruidSpellcasting(character,selections={}){
  try{
    ...
    const cantripPool=base.filter(spell=>spell.level===0).map(spell=>spell.id),cantrips=resolveSpellChoices({available:cantripPool,selected:selections.cantrips||[],required:validation.limits.cantrips,label:"Druid cantrips"});
