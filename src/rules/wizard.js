...
function buildSpellbook(character,spells,selections) {
  try {
    ...
    const mandatory=uniqueStrings([...(selections.spellbook||[]),...(selections.prepared||[])]), duplicates=duplicateValues(selections.spellbook||[]);
    ...
    for (const slot of fillOrder) {
      ...
      const pool=levelSpells.filter(spell=>spell.level<=slot.maxLevel&&(!slot.school||spell.school===slot.school)&&!used.has(spell.id));
      if (!pool.length) throw new Error(`No legal spell remains for ${slot.label}`);
      assignments.set(slot.index,pick(pool).id);
    }
    const all=slots.map((slot,index)=>assignments.get(index));
    return {selected:[...(selections.spellbook||[])],randomized:all.filter(id=>!(selections.spellbook||[]).includes(id)),all,acquisition:slots.map((slot,index)=>({label:slot.label,spellId:assignments.get(index)}))};
