...
export const DEFAULT_PACT_WEAPON_ID="sickle";
...
export function warlockPactWeaponId(character){
  try{
    ...
    return (character.equipment?.weapons||[]).find(weaponId=>!RANGED_WEAPON_IDS.has(weaponId))||DEFAULT_PACT_WEAPON_ID;
...
export function warlockWeaponAttack(character,id,weapon,proficiencyBonus,{attackStyleBonus=0,damageStyleBonus=0}={}){
  try{
    ...
    return{...weapon,id,ability,attackBonus:mod+proficiencyBonus+attackStyleBonus,damageBonus:mod+damageStyleBonus,pactWeapon:usesCharisma,conjuredPactWeapon:usesCharisma&&!(character.equipment?.weapons||[]).includes(id)};
