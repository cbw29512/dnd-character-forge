...
export function applyRogueAdvancement(character,data,selections={}){
  try{
    ...
    if(character.ruleset==="2014"){
      ...
      if(character.level>=19){const selected=selections[19]??RANDOM;if(selected!==RANDOM&&!EPIC_BOONS.includes(selected))throw new Error(`Illegal Rogue Epic Boon selection: ${selected}`);const feat=requiredFeat(data,selected===RANDOM?pick(EPIC_BOONS):selected),ability=bestAbility(next.abilities,["dex","con","wis","int","cha","str"],30);next.abilities[ability]+=1;next.abilityMaximums[ability]=Math.max(next.abilityMaximums[ability]||20,next.abilities[ability]);next.feats.push(feat);choices.push({level:19,type:"epic-boon",id:feat.id,name:feat.name,increases:{[ability]:1},locked:selected!==RANDOM});}
