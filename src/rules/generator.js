...
import { applyBarbarianAdvancement } from "./barbarian-advancement.js";
...
export function generateCharacter(state){
  try{
    ...
    const data=dataFor(state.ruleset),species=resolve(state.constraints.species,data.species),requestedLevel=state.constraints.level===RANDOM?null:Number(state.constraints.level),classPool=requestedLevel?data.classes.filter(item=>requestedLevel<=(item.maxLevel||5)):data.classes,cls=resolve(state.constraints.class,classPool),background=resolve(state.constraints.background,data.backgrounds);if(!species||!cls||!background)throw new Error("A selected character option is unavailable for this ruleset/level.");
    ...
    let abilities=generateBaseAbilities("standard",cls.abilityPriority);abilities=state.ruleset==="2014"?apply2014Species(abilities,species):apply2024Background(abilities,background,cls.primary);if(cls.id!=="barbarian")abilities=applyClassAsi(abilities,level,cls.primary);
    ...
    let character={id:crypto.randomUUID(),sourceMode:SOURCE.RAW,ruleset:state.ruleset,level,name:state.constraints.name.trim()||pick(NAMES),species,size:resolveSize(state.ruleset,species),class:cls,subclass,background,abilities,abilityMaximums:Object.fromEntries(ABILITIES.map(a=>[a,20])),skills,expertise:[],saves:cls.saves,proficiency:proficiencyBonus(level),equipment,fightingStyle,divineOrder,languages:resolveLanguages(state.ruleset,background),feats:[],homebrew:[],homebrewAcBonus:0,features,spells:null,classResources:cls.id==="barbarian"?barbarianResources(state.ruleset,level):[],advancementChoices:[]};
