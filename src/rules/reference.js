...
function dynamicFeat(character,feat){
  try{
    ...
    if(character.ruleset==="2024"&&feat.id==="boon-fate")return rr("Epic Boon","After a D20 Test",`The +1 ${pretty(character.epicBoonAbility)} increase and maximum of 30 are already applied. When you or a creature within 60 ft succeeds or fails a D20 Test, roll 2d4 and add or subtract the total from the roll. Improve Fate refreshes after you roll Initiative or finish a Short or Long Rest.`);
    return null;
...
function dynamicFeature(character,name){
  try{
    ...
    if(name==="Empowered Evocation")return rr("Evoker","Evocation damage",`Add your Intelligence modifier (${signed(abilityMod(character.abilities.int))}) to one damage roll of each Wizard Evocation spell you cast.`);
    ...
    if(name==="Channel Divinity")return channelDivinity(character);
    if(name==="Divine Spark")return divineSpark(character);
    ...
    if(name==="Blessed Healer")return blessedHealer(character);
    if(name==="Blessed Strikes")return blessedStrikes(character,false);
    if(name==="Divine Intervention")return divineIntervention(character);
    if(name==="Improved Blessed Strikes")return blessedStrikes(character,true);
    if(name==="Supreme Healing")return supremeHealing(character);
    if(name==="Greater Divine Intervention")return greaterDivineIntervention(character);
...
function channelDivinity(character){try{const uses=character.cleric?.channelDivinityUses;if(!Number.isInteger(uses)||uses<1)throw new Error("Channel Divinity reference requires active Cleric progression data.");return rr("Cleric","Resource",`${uses} uses fuel Divine Spark, Turn Undead, or a subclass Channel Divinity option. Regain one expended use after a Short Rest and all expended uses after a Long Rest.`);}catch(error){console.error("[reference] Channel Divinity failed",error);throw error;}}
function divineSpark(character){try{const dice=character.cleric?.divineSparkDice,wis=abilityMod(character.abilities.wis);if(!Number.isInteger(dice)||dice<1)throw new Error("Divine Spark reference requires active Cleric progression data.");return rr("Cleric","Magic action · Channel Divinity",`Choose another creature you can see within 30 ft. Roll ${dice}d8 ${signed(wis)}: restore that many Hit Points, or force a Constitution save against DC ${character.spells.saveDc} for that much Radiant or Necrotic damage; a successful save takes half.`);}catch(error){console.error("[reference] Divine Spark failed",error);throw error;}}
...
function blessedHealer(character){try{return rr("Life Domain","Healing spell with slot",`When a spell you cast with a spell slot restores Hit Points to a creature other than you, you also regain Hit Points equal to 2 + the slot's level.`);}catch(error){console.error("[reference] Blessed Healer failed",error);throw error;}}
function blessedStrikes(character,improved){try{const wis=abilityMod(character.abilities.wis);if(character.blessedStrikes==="divine-strike")return rr("Cleric",improved?"Once per turn · weapon hit":"Once per turn · weapon hit",`Divine Strike adds ${improved?"2d8":"1d8"} Radiant or Necrotic damage to one weapon hit on your turn.`);if(character.blessedStrikes==="potent-spellcasting"){const extra=improved?` When a Cleric cantrip deals damage, you can also give yourself or one creature within 60 ft ${Math.max(0,2*wis)} Temporary Hit Points.`:"";return rr("Cleric","Cleric cantrip damage",`Potent Spellcasting adds your Wisdom modifier (${signed(wis)}) to the damage of Cleric cantrips.${extra}`);}throw new Error("Blessed Strikes reference requires a legal choice.");}catch(error){console.error("[reference] Blessed Strikes failed",error);throw error;}}
function divineIntervention(character){try{return rr("Cleric","Magic action · Long Rest",`Choose a Cleric spell of level 5 or lower that does not require a Reaction to cast. As this Magic action, cast it without expending a spell slot or providing Material components. After using Divine Intervention, you cannot use it again until you finish a Long Rest.`);}catch(error){console.error("[reference] Divine Intervention failed",error);throw error;}}
function supremeHealing(character){try{return rr("Life Domain","Healing roll",`Whenever you would roll one or more dice to restore Hit Points with a spell or Channel Divinity, use the highest possible result on each healing die instead.`);}catch(error){console.error("[reference] Supreme Healing failed",error);throw error;}}
function greaterDivineIntervention(character){try{return rr("Cleric","Divine Intervention",`Divine Intervention can now choose Wish. If you use it to cast Wish, you cannot use Divine Intervention again until you finish 2d4 Long Rests.`);}catch(error){console.error("[reference] Greater Divine Intervention failed",error);throw error;}}
