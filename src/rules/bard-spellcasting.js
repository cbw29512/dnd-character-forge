...
export function validateBardSpellSelections(character,selections={}){
  try{
    ...
    if(ruleset==="2014"){
      ...
      const badKnown=known.filter(id=>!baseLeveled.has(id)),badSecrets=magicalSecrets.filter(id=>!secretAnyLevel.has(id));if(badKnown.length)throw new Error(`Illegal Bard known-spell selection: ${badKnown.join(", ")}.`);if(badSecrets.length)throw new Error(`Illegal Magical Secrets selection: ${badSecrets.join(", ")}.`);return{valid:true,maxLevel,normalKnown,standardSecrets,loreCount:expectedLore,nonBardCapacity:null};
    }
    if(ruleset!=="2024")throw new Error(`Unsupported Bard spell ruleset: ${ruleset}.`);if(known.length)throw new Error("2024 Bard uses prepared spells, not spells-known selections.");if(character.level<10&&magicalSecrets.length)throw new Error("2024 Magical Secrets is unavailable before Bard level 10.");
    ...
    const combinedPrepared=[...prepared,...magicalSecrets];if(combinedPrepared.length>progression.prepared)throw new Error(`Choose at most ${progression.prepared} prepared Bard spells.`);const allowed=character.level>=10?secretLeveled:baseLeveled,badPrepared=combinedPrepared.filter(id=>!allowed.has(id));if(badPrepared.length)throw new Error(`Illegal Bard prepared-spell selection: ${badPrepared.join(", ")}.`);
...
export function buildBardSpellcasting(character,selections={}){
  try{
    ...
    const words=bardAlwaysPrepared(character),loreExcluded=uniqueStrings([...cantrips.all,...words]),loreAvailable=without(lorePool.filter(spell=>spell.level===0||spell.level<=maxLevel).map(spell=>spell.id),loreExcluded),loreDiscoveries=resolveSpellChoices({available:loreAvailable,selected:selections.loreDiscoveries||[],required:validation.loreCount,label:character.ruleset==="2014"?"Additional Magical Secrets":"Magical Discoveries"});
    if(character.ruleset==="2014"){
      const knownPool=without(base.filter(spell=>spell.level>0&&spell.level<=maxLevel).map(spell=>spell.id),[...cantrips.all,...loreDiscoveries.all]),normalKnown=resolveSpellChoices({available:knownPool,selected:selections.known||[],required:validation.normalKnown,label:"known Bard spells"}),secretPool=without(allSecrets.filter(spell=>spell.level===0||spell.level<=maxLevel).map(spell=>spell.id),[...cantrips.all,...normalKnown.all,...loreDiscoveries.all]),magicalSecrets=resolveSpellChoices({available:secretPool,selected:selections.magicalSecrets||[],required:validation.standardSecrets,label:"Magical Secrets"});
    ...
    const selectedPrepared=uniqueStrings([...(selections.prepared||[]),...(selections.magicalSecrets||[])]),prepared=resolve2024Prepared({base,allSecrets,maxLevel,required:p.prepared,selected:selectedPrepared,excluded:[...cantrips.all,...loreDiscoveries.all,...words],outsideCapacity:validation.nonBardCapacity}),baseIds=new Set(base.map(spell=>spell.id)),magicalSecrets=prepared.all.filter(id=>!baseIds.has(id)),alwaysAll=uniqueStrings([...words,...loreDiscoveries.all]),all=uniqueStrings([...cantrips.all,...prepared.all,...alwaysAll].map(id=>namesById.get(id)||id));
...
function without(pool,excluded){const blocked=new Set(excluded||[]);return pool.filter(id=>!blocked.has(id));}
