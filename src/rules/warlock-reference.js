...
function featureReference(c,name){
  try{
    ...
    if(name==="Dark One's Blessing"){const amount=Math.max(1,(c.abilities?.cha!=null?Math.floor((c.abilities.cha-10)/2):0)+c.level);return c.ruleset==="2014"?rr("The Fiend","When you reduce a hostile creature to 0 HP",`Gain ${amount} Temporary Hit Points (Charisma modifier + Warlock level, minimum 1).`):rr("Fiend Patron","When you reduce an enemy to 0 HP",`Gain ${amount} Temporary Hit Points when you reduce an enemy to 0 HP. You also gain this benefit when someone else reduces an enemy within 10 feet of you to 0 HP.`);}
    ...
    if(name==="Hurl Through Hell")return c.ruleset==="2014"?rr("The Fiend","Attack hit · once per Long Rest","After you hit a creature, banish it through a nightmarish realm until the end of your next turn; a non-fiend takes 10d10 Psychic damage when it returns."):rr("Fiend Patron","Once per turn after attack hit","Force a Charisma save; on failure the target vanishes, immediately takes 8d10 Psychic damage if it isn't a Fiend, and is Incapacitated until the end of your next turn, when it returns. Restore the use after a Long Rest or by expending a Pact Magic slot.");
