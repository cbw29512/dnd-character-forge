...
function styleReference(c,style){
  try{
    if(style.id==="blessed-warrior")return rr("Fighting Style","Cantrips",`Learn two Cleric cantrips as Paladin spells using Charisma: ${c.spells?.cantrips?.all?.join(", ")||"two legal Cleric cantrips"}. Whenever you gain a Paladin level, replace one with another Cleric cantrip.`);
