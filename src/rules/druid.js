...
export const LAND_2014=Object.freeze(["arctic","coast","desert","forest","grassland","mountain","swamp"]);
...
export function druidProgressionFor(ruleset,level,subclassId=null){
  try{
    ...
    if(ruleset==="2014")return Object.freeze({cantrips:CANTRIPS_2014[value]+(circle&&value>=2?1:0),prepared:null,slots:Object.freeze({...SLOTS[value]}),wildShapeUses:value<2?0:2,unlimitedWildShape:value>=20,knownFormCount:null,maxCr:maxCr(value),allowSwim:value>=4,allowFly:value>=8,durationHours:value<2?0:Math.floor(value/2),naturalRecovery:circle&&value>=2?Math.ceil(value/2):0,landsStride:circle&&value>=6,naturesWard:circle&&value>=10,naturesSanctuary:circle&&value>=14,timelessBody:value>=18,beastSpells:value>=18,archdruid:value>=20});
    return Object.freeze({cantrips:CANTRIPS_2024[value],prepared:PREPARED_2024[value],slots:Object.freeze({...SLOTS[value]}),wildShapeUses:value<2?0:value<6?2:value<17?3:4,unlimitedWildShape:false,knownFormCount:value<2?0:value<4?4:value<8?6:8,maxCr:maxCr(value),allowSwim:value>=2,allowFly:value>=8,durationHours:value<2?0:Math.floor(value/2),wildCompanion:value>=2,wildResurgence:value>=5,elementalFury:value>=7,improvedElementalFury:value>=15,beastSpells:value>=18,epicBoon:value>=19,archdruid:value>=20,evergreenWildShape:value>=20,natureMagician:value>=20,circleLand:circle&&value>=3,landsAidDice:circle&&value>=3?(value>=14?4:value>=10?3:2):0,naturalRecovery:circle&&value>=6?Math.ceil(value/2):0,naturesWard:circle&&value>=10,naturesSanctuary:circle&&value>=14});
