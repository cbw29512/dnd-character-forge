...
export function bindPregenLibrary(options={}){
  try{
    callbacks=options;["pregenSearch","pregenRuleset"].forEach(id=>document.getElementById(id)?.addEventListener("input",renderPregenLibrary));
...
export function renderPregenLibrary(){
  try{
    ...
    const items=loadPregens().filter(item=>{const haystack=`${item.name} ${item.className} ${item.speciesName} ${item.backgroundName}`.toLowerCase();return item.sourceMode==="RAW"&&!item.character?.homebrew?.length&&(!search||haystack.includes(search))&&(ruleset==="all"||item.ruleset===ruleset);});
    const grid=document.getElementById("pregenGrid"),count=document.getElementById("pregenCount");if(count)count.textContent=`${items.length} RAW saved`;if(!grid)return;
    grid.innerHTML=items.length?items.map(card).join(""):`<div class="library-empty"><span>✦</span><h3>No matching RAW pregens yet</h3><p>Forge a character, then choose <strong>Save to Pregens</strong>. Exact mechanical duplicates are blocked automatically.</p></div>`;
  }catch(error){console.error("[library-ui] render failed",error);throw error;}
}
function card(item){
  try{return `<article class="library-card"><div class="library-card-top"><span class="library-badge raw">✓ RAW</span><span class="library-level">Level ${item.level}</span></div><h3>${escapeHtml(item.name)}</h3><p>${escapeHtml(item.speciesName)} ${escapeHtml(item.className)} · ${escapeHtml(item.backgroundName)}</p><div class="library-meta"><span>${escapeHtml(item.ruleset)}</span><span>Fingerprint ${item.fingerprint.slice(0,8)}</span></div><div class="library-actions"><button class="library-open" type="button" data-view-pregen="${item.id}">Open character</button><button class="library-remove" type="button" data-remove-pregen="${item.id}">Remove</button></div></article>`;}
