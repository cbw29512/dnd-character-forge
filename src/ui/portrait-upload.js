const MAX_SOURCE_BYTES=8*1024*1024,MAX_SIDE=560,OUTPUT_QUALITY=.82,ALLOWED=new Set(["image/jpeg","image/png","image/webp"]);
...
function ensurePanel(){
  try{let panel=document.getElementById("portraitPanel");if(panel)return panel;const anchor=document.getElementById("spellPickerPanel");if(!anchor)throw new Error("Portrait panel anchor is unavailable.");panel=document.createElement("div");panel.id="portraitPanel";panel.className="portrait-panel";panel.innerHTML=`<div class="portrait-heading"><div><strong>Character portrait</strong><small>Optional · used only on the premium printable sheet.</small></div><span class="portrait-badge">Presentation</span></div><div class="portrait-body"><div class="portrait-preview-wrap"><img id="portraitPreview" alt="Character portrait preview" hidden><div id="portraitEmpty" class="portrait-empty"><strong>Class crest</strong><span>Upload art to replace the automatic crest.</span></div></div><div class="portrait-actions"><label class="portrait-upload-button">Choose image<input id="portraitFile" type="file" accept="image/jpeg,image/png,image/webp"></label><button id="removePortrait" class="portrait-remove" type="button" hidden>Remove portrait</button><small>JPEG, PNG, or WebP · resized locally before saving.</small></div></div>`;anchor.parentNode.insertBefore(panel,anchor);return panel;}
  catch(error){console.error("[portrait] panel creation failed",error);throw error;}
}
async function normalizePortrait(file){
  try{if(!ALLOWED.has(file.type))throw new Error("Portrait must be a JPEG, PNG, or WebP image.");if(file.size>MAX_SOURCE_BYTES)throw new Error("Portrait source image must be 8 MB or smaller.");const url=await readFile(file),image=await loadImage(url),scale=Math.min(1,MAX_SIDE/Math.max(image.naturalWidth,image.naturalHeight)),canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(image.naturalWidth*scale));canvas.height=Math.max(1,Math.round(image.naturalHeight*scale));const context=canvas.getContext("2d");if(!context)throw new Error("This browser cannot prepare portrait images.");context.fillStyle="#eadcc2";context.fillRect(0,0,canvas.width,canvas.height);context.drawImage(image,0,0,canvas.width,canvas.height);const output=canvas.toDataURL("image/jpeg",OUTPUT_QUALITY);if(!validPortrait(output))throw new Error("Portrait conversion failed.");return output;}
