...
export function populateOptions(state){
  try{const data=dataFor(state);fill("species",rawOnly(data.species));fill("class",rawOnly(data.classes));fill("background",rawOnly(data.backgrounds));populateLevels(state);populateSubclasses(state);renderRandomBackgroundCoverage(state);}
  catch(error){console.error("[ui] populateOptions failed",error);throw error;}
}
export function populateLevels(state){
  try{
    const data=dataFor(state),classId=state.constraints.class,cls=data.classes.find(item=>item.id===classId&&item?.contentKind!=="forge-original");
    const maxLevel=cls?.maxLevel||Math.min(...rawOnly(data.classes).map(item=>item.maxLevel||5));
...
export function populateSubclasses(state){
  try{
    const data=dataFor(state),classId=state.constraints.class,cls=data.classes.find(item=>item.id===classId&&item?.contentKind!=="forge-original"),level=state.constraints.level;
    const levelAllows=!cls||level==="random"||Number(level)>=cls.subclassLevel;
    const items=cls&&levelAllows?rawOnly(data.subclasses.filter(item=>item.classId===classId)):[];
