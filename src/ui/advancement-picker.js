...
import { MONK_ADVANCEMENT } from "../rules/monk-advancement.js";
...
export function refreshAdvancementPicker(state){
  try{
    const panel=document.getElementById("advancementPanel"),fields=document.getElementById("advancementFields"),summary=document.getElementById("advancementSummary"),classId=state.constraints.class,isSupported=["barbarian","rogue","fighter","monk"].includes(classId);panel.hidden=!isSupported;if(!isSupported){fields.innerHTML="";return;}
...
function levelsFor(classId,ruleset){
  if(classId==="barbarian")return ruleset==="2014"?BARBARIAN_ADVANCEMENT.levels2014:[...BARBARIAN_ADVANCEMENT.normal2024,19];
  if(classId==="rogue")return ruleset==="2014"?ROGUE_ADVANCEMENT.levels2014:[...ROGUE_ADVANCEMENT.levels2024,19];
  if(classId==="monk")return ruleset==="2014"?MONK_ADVANCEMENT.levels2014:[...MONK_ADVANCEMENT.normal2024,19];
  return ruleset==="2014"?FIGHTER_ADVANCEMENT.levels2014:[...FIGHTER_ADVANCEMENT.levels2024,19];
}
function advancementFor(classId){if(classId==="barbarian")return BARBARIAN_ADVANCEMENT;if(classId==="rogue")return ROGUE_ADVANCEMENT;if(classId==="monk")return MONK_ADVANCEMENT;return FIGHTER_ADVANCEMENT;}
function fieldFor(state,classId,level,grapplerLevel){const isEpic=state.ruleset==="2024"&&level===19,advancement=advancementFor(classId),options=isEpic?[[RANDOM,"Random"],...advancement.epicBoons.map(id=>[id,BOON_NAMES[id]])]:state.ruleset==="2014"?(classId==="rogue"?ROGUE_2014:ASI_GRAPPLER_2014):GENERAL_2024,current=state.advancementSelections?.[level]??RANDOM;return `<label class="advancement-field"><span>Level ${level}${isEpic?" · Epic Boon":""}</span><select data-advancement-level="${level}">${options.map(([value,name])=>`<option value="${value}"${value===current?" selected":""}${value==="grappler"&&grapplerLevel&&grapplerLevel!==String(level)?" disabled":""}>${name}</option>`).join("")}</select></label>`;}
