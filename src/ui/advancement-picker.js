...
const BARBARIAN_2014=[[RANDOM,"Random"],["asi","Ability Score Improvement"],["grappler","Grappler"]];
...
export function refreshAdvancementPicker(state){
  try{
    const panel=document.getElementById("advancementPanel"),fields=document.getElementById("advancementFields"),summary=document.getElementById("advancementSummary"),classId=state.constraints.class,isSupported=["barbarian","rogue"].includes(classId);panel.hidden=!isSupported;if(!isSupported){fields.innerHTML="";return;}
...
export function resetAdvancementSelections(state){try{state.advancementSelections={4:RANDOM,8:RANDOM,10:RANDOM,12:RANDOM,16:RANDOM,19:RANDOM};refreshAdvancementPicker(state);}catch(error){console.error("[advancement-picker] reset failed",error);throw error;}}
function levelsFor(classId,ruleset){if(classId==="barbarian")return ruleset==="2014"?BARBARIAN_ADVANCEMENT.levels2014:[...BARBARIAN_ADVANCEMENT.normal2024,19];return ruleset==="2014"?ROGUE_ADVANCEMENT.levels2014:[...ROGUE_ADVANCEMENT.levels2024,19];}
function fieldFor(state,classId,level,grapplerLevel){const isEpic=state.ruleset==="2024"&&level===19,boons=classId==="barbarian"?BARBARIAN_ADVANCEMENT.epicBoons:ROGUE_ADVANCEMENT.epicBoons,options=isEpic?[[RANDOM,"Random"],...boons.map(id=>[id,BOON_NAMES[id]])]:state.ruleset==="2014"?(classId==="barbarian"?BARBARIAN_2014:ROGUE_2014):GENERAL_2024,current=state.advancementSelections?.[level]??RANDOM;return `<label class="advancement-field"><span>Level ${level}${isEpic?" · Epic Boon":""}</span><select data-advancement-level="${level}">${options.map(([value,name])=>`<option value="${value}"${value===current?" selected":""}${value==="grappler"&&grapplerLevel&&grapplerLevel!==String(level)?" disabled":""}>${name}</option>`).join("")}</select></label>`;}
