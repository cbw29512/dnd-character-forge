...
function portrait(m){const art=m.portraitDataUrl?`<div class="ps-dossier-portrait-art has-image"><img src="${esc(m.portraitDataUrl)}" alt="" style="${portraitImageStyle(m)}"></div>`:`<div class="ps-dossier-portrait-art class-${esc(m.identity.classId)}">${classPlaceholderArt(m.identity.classId)}<small>${esc(m.theme.label)}</small></div>`;return `<section class="ps-dossier-portrait">${art}</section>`;}
function story(d){return `<section class="ps-dossier-story"><h2>Backstory</h2>${d.backstory.map(text=>`<p>${esc(text)}</p>`).join("")}</section>`;}
