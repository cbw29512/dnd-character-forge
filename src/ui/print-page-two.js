...
function groupByLevel(entries){
  try{
    ...
    for(const entry of entries){const list=grouped.get(entry.level)||[];list.push(entry);grouped.set(entry.level,list);}
    return grouped;
  }catch(error){
    console.error("[print-page-two] could not group spells by level",error);
    throw error;
  }
}
function balancedLevelColumns(byLevel){
  try{
    let best=null;
    for(let first=1;first<=SPELL_LEVELS.length-2;first++){
      for(let second=first+1;second<=SPELL_LEVELS.length-1;second++){
        const columns=[SPELL_LEVELS.slice(0,first),SPELL_LEVELS.slice(first,second),SPELL_LEVELS.slice(second)];
        const loads=columns.map(levels=>levels.reduce((total,level)=>total+2+Math.max(1,(byLevel.get(level)||[]).length),0));
        const maxLoad=Math.max(...loads),minLoad=Math.min(...loads),score=(maxLoad*100)+(maxLoad-minLoad);
        if(!best||score<best.score)best={columns,score};
      }
    }
    return best?.columns||FALLBACK_LEVEL_COLUMNS;
  }catch(error){
    console.error("[print-page-two] could not balance spell-level columns; using safe fallback",error);
    return FALLBACK_LEVEL_COLUMNS;
  }
}
