...
export function renderPrintPageTwo(m){
  try{
    ...
    const s=m.spellPage;
    return `${sheetArticleOpen(m,"ps-caster-page")}<div class="ps-frame ps-caster-frame">${classWatermark(m)}<header class="ps-caster-header"><div><small>${esc(m.theme.label)} spell record</small><h1>${esc(m.identity.name)}</h1><p>${esc(m.identity.className)} ${m.identity.level}${m.identity.subclassName?` · ${esc(m.identity.subclassName)}`:""}</p></div><div class="ps-caster-stats"><span>Spellcasting Ability<b>${esc(s.ability)}</b></span><span>Spell Save DC<b>${s.saveDc}</b></span><span>Spell Attack Bonus<b>${esc(s.attackBonus)}</b></span></div></header><main class="ps-caster-body">${spellSection(m,s)}${rulesSection(m)}${auditSection(m)}</main><footer class="ps-footer"><span>${esc(s.source)}</span><span class="ps-audit">✓ ${esc(m.audit.status)} · ${esc(m.audit.version)} · RAW integrity · Page 2/2</span></footer></div></article>`;
  }catch(error){console.error("[print-page-two] render failed",error);throw error;}
}
function spellSection(m,s){try{const byLevel=groupByLevel(s.entries),levelColumns=balancedLevelColumns(byLevel);return `<section class="ps-caster-spells"><div class="ps-section-title"><h2>Spells</h2><span>Prepared, known, and slot tracking follow the familiar 5e sheet flow</span></div><p class="ps-spell-key"><b>●</b> Prepared · <b>K</b> Known · <b>A</b> Always Prepared · <b>M</b> Mastery · <b>S</b> Signature · <b>B</b> Spellbook</p><div class="ps-spell-level-grid">${levelColumns.map(levels=>`<div class="ps-spell-level-column">${levels.map(level=>spellLevel(level,byLevel.get(level)||[],slotCountFor(s.slots,level))).join("")}</div>`).join("")}</div>${renderPaladinSpellSupport(m)}${renderRangerSpellSupport(m)}${renderDruidSpellSupport(m)}</section>`;}catch(error){console.error("[print-page-two] spell section failed",error);throw error;}}
