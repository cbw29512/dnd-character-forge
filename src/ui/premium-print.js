...
export function renderPremiumPrintSheet(character,target){
  try{
    ...
    target.innerHTML=forgeOriginal
      ?packet.replace(/ · RAW · /g," · 5E Compatible · ").replace(/RAW integrity|Compatible content/g,"Character Forge Original · Compatible content").replace(/Custom content/g,"Character Forge Original · Custom content")
      :packet;
    return model;
  }catch(error){
