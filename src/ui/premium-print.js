...
export function renderPremiumPrintSheet(character,target){
  try{
    ...
    const certification=certificationFooterText(character);
    const packet=`${renderPrintPageOne(model)}${renderPrintPageTwo(model)}${renderPrintDossier(model)}`.replace(/<span class="ps-audit">/g,`<span class="ps-audit">${certification} · `);
