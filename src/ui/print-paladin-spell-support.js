...
export function renderPaladinSpellSupport(model){
  try{
    ...
    const utility=model.classUtility,ruleset=String(model.audit?.rulesLabel||"").startsWith("2014")?"2014":"2024",is2014=ruleset==="2014",stats=Object.fromEntries((utility.stats||[]).map(item=>[item.label,item]));
    const lay=stats["Lay On Hands"],channel=stats.Channel,aura=stats.Aura,always=model.spellcasting?.alwaysPrepared?.length||0;
    const cards=is2014?[
      card("Divine Smite","On a melee weapon hit","Spend a spell slot for +2d8 Radiant at level 1, +1d8 per slot level above 1; add another +1d8 against a Fiend or Undead."),
