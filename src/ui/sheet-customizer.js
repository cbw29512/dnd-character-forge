...
const CONTROL_IDS=["sheetStyle","sheetPaper","sheetOrnament","sheetFrame","sheetPrintMode","portraitFilter","portraitVisible","portraitX","portraitY","portraitZoom"];
...
export function bindSheetCustomizer(state,showToast){
  try{
    ...
    panel.querySelector("#resetSheetStyle").addEventListener("click",()=>{
      try{state.sheetCustomization=createDefaultSheetCustomization();applySheetCustomizationToCurrent(state);renderSheetCustomizer(state);showToast("Sheet presentation reset to premium class defaults.");}
...
export function renderSheetCustomizer(state){
  try{
    ...
    setValue(panel,"sheetStyle",value.style);setValue(panel,"sheetPaper",value.paper);setValue(panel,"sheetOrnament",value.ornament);setValue(panel,"sheetFrame",value.frame);setValue(panel,"sheetPrintMode",value.printMode);setValue(panel,"portraitFilter",value.portraitFilter);
...
function updateStateFromPanel(state,panel){
  try{
    state.sheetCustomization={...normalizeSheetCustomization({
      style:panel.querySelector("#sheetStyle").value,paper:panel.querySelector("#sheetPaper").value,ornament:panel.querySelector("#sheetOrnament").value,frame:panel.querySelector("#sheetFrame").value,printMode:panel.querySelector("#sheetPrintMode").value,portraitFilter:panel.querySelector("#portraitFilter").value,portraitVisible:panel.querySelector("#portraitVisible").checked,portraitX:panel.querySelector("#portraitX").value,portraitY:panel.querySelector("#portraitY").value,portraitZoom:panel.querySelector("#portraitZoom").value
    })};
