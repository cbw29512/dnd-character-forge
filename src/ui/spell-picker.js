...
export function bindSpellPicker(state, showToast) {
  try {
    const panel=document.getElementById("spellPickerPanel"), button=document.getElementById("openSpellPicker"), dialog=document.getElementById("spellDialog");
    ...
    ["class","level","ruleset"].forEach(id=>document.getElementById(id).addEventListener("change",()=>{
      state.spellSelections={cantrips:[],spellbook:[],prepared:[]}; refreshSpellPicker(state);
    }));
...
export function refreshSpellPicker(state) {
  try {
    const panel=document.getElementById("spellPickerPanel"), button=document.getElementById("openSpellPicker");
    const wizard=state.constraints.class==="wizard"; panel.hidden=!wizard;
    if(!wizard)return;
    ...
    document.getElementById("spellPickerHint").textContent=hasLevel?"Pick any spells you want. Forge randomizes every remaining legal choice.":"Leave Level on Random for a fully randomized spell loadout, or choose a level to pick specific spells.";
    updateSummary(state);
...
function renderPicker(state) {
  try {
    ...
    const root=document.getElementById("spellPickerLists");
    root.innerHTML=Object.keys(LABELS).map(bucket=>section(bucket,spells,state)).join("");
    document.getElementById("spellSearch").value=""; updateSummary(state);
  } catch(error){ console.error("[spell-picker] render failed",error); throw error; }
}
function section(bucket,spells,state) {
  try {
    const pool=bucket==="cantrips"?spells.filter(spell=>spell.level===0):spells.filter(spell=>spell.level>0);
    const selected=new Set(state.spellSelections[bucket]);
...
function updateSummary(state) {
  try {
    const total=Object.values(state.spellSelections).reduce((sum,values)=>sum+values.length,0), summary=document.getElementById("spellChoiceSummary");
    if(summary)summary.textContent=total?`${total} spell choice${total===1?"":"s"} fixed · all remaining choices Random`:"All spell choices Random";
