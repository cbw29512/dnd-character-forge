...
test("feature registry contains only currently verified classes", () => {
  try {
    assert.deepEqual([...supportedFeatureClasses()].sort(), ["cleric", "fighter", "wizard"]);
  } catch (error) {
    console.error("[test] feature registry classes", error);
    throw error;
  }
});
...
test("feature registry fails closed for an unimplemented class", () => {
  try {
    assert.throws(
      () => resolveClassFeatures({ ruleset: "2024", classId: "barbarian", level: 1 }),
      /not implemented/i
    );
  } catch (error) {
    console.error("[test] feature registry fail closed", error);
    throw error;
  }
});

test("feature registry resolves existing Fighter, Wizard, and Cleric features", () => {
  try {
    ...
    console.error("[test] feature registry supported classes", error);
    throw error;
  }
});
