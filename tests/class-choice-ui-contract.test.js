...
const baseKeys=fields=>keys(fields).filter(key=>key!=="advancements");
...
test("Class Options exposes every implemented Bard, Sorcerer, Warlock, Druid, Ranger, Monk, Paladin, and Cleric choice family",()=>{
  const bard=classChoiceFieldsForState(stateFor("2014","bard",20,"college-lore"));assert.deepEqual(baseKeys(bard),["instruments","loreBonusSkills","expertise"]);assert.equal(field(bard,"instruments").type,"multi");assert.equal(field(bard,"expertise").max,4);assert.ok(advancementFields(bard).length>0);assert.ok(advancementFields(bard).every(item=>item.type==="indexed"&&item.defaultLabel==="Automatic ASI"));
  const sorcerer=classChoiceFieldsForState(stateFor("2024","sorcerer",20,"draconic-sorcery"));assert.deepEqual(baseKeys(sorcerer),["metamagic","elementalAffinity"]);assert.equal(field(sorcerer,"metamagic").max,6);assert.ok(advancementFields(sorcerer).length>0);
  const warlock2014=classChoiceFieldsForState(stateFor("2014","warlock",20,"fiend"));assert.deepEqual(baseKeys(warlock2014),["pactBoon","eldritchInvocations"]);assert.equal(field(warlock2014,"eldritchInvocations").max,8);assert.ok(advancementFields(warlock2014).length>0);
  const warlock2024=classChoiceFieldsForState(stateFor("2024","warlock",20,"fiend-patron"));assert.deepEqual(baseKeys(warlock2024),["eldritchInvocations"]);assert.equal(field(warlock2024,"eldritchInvocations").max,10);assert.ok(advancementFields(warlock2024).length>0);
  const druid=classChoiceFieldsForState(stateFor("2024","druid",20,"circle-land"));for(const key of ["primalOrder","circleLand","elementalFury","knownForms"])assert.ok(keys(druid).includes(key),`missing Druid ${key}`);assert.equal(field(druid,"knownForms").max,8);assert.ok(advancementFields(druid).length>0);
  ...
  const ranger2024=classChoiceFieldsForState(stateFor("2024","ranger",20,"hunter"));for(const key of ["fightingStyle","huntersPrey","defensiveTactics"])assert.ok(keys(ranger2024).includes(key),`missing 2024 Ranger ${key}`);assert.ok(advancementFields(ranger2024).length>0);
  const monk=classChoiceFieldsForState(stateFor("2024","monk",20,"open-hand")),paladin=classChoiceFieldsForState(stateFor("2024","paladin",20,"oath-devotion")),cleric=classChoiceFieldsForState(stateFor("2024","cleric",20,"life-domain"));assert.ok(keys(monk).includes("monkTool"));assert.ok(keys(paladin).includes("fightingStyle"));for(const key of ["divineOrder","blessedStrikes"])assert.ok(keys(cleric).includes(key));for(const fields of [monk,paladin,cleric])assert.ok(advancementFields(fields).length>0);
});
