...
test("Class Options exposes every implemented Bard, Sorcerer, Warlock, Druid, Ranger, Monk, Paladin, and Cleric choice family",()=>{
  ...
  const sorcerer=classChoiceFieldsForState(stateFor("2024","sorcerer",20,"draconic-sorcery"));assert.deepEqual(baseKeys(sorcerer),["classSkills","metamagic","elementalAffinity"]);assert.equal(field(sorcerer,"metamagic").max,6);assert.ok(advancementFields(sorcerer).length>0);
  ...
  const warlock2024=classChoiceFieldsForState(stateFor("2024","warlock",20,"fiend-patron"));assert.deepEqual(baseKeys(warlock2024),["classSkills","eldritchInvocations"]);assert.equal(field(warlock2024,"eldritchInvocations").max,10);assert.ok(advancementFields(warlock2024).length>0);
  const druid=classChoiceFieldsForState(stateFor("2024","druid",20,"circle-land"));for(const key of ["classSkills","primalOrder","circleLand","elementalFury","knownForms"])assert.ok(keys(druid).includes(key),`missing Druid ${key}`);assert.equal(field(druid,"knownForms").max,8);assert.ok(advancementFields(druid).length>0);
  ...
  const ranger2024=classChoiceFieldsForState(stateFor("2024","ranger",20,"hunter"));for(const key of ["classSkills","fightingStyle","expertise","deftExplorerLanguages","huntersPrey","defensiveTactics"])assert.ok(keys(ranger2024).includes(key),`missing 2024 Ranger ${key}`);assert.ok(advancementFields(ranger2024).length>0);
  const monk=classChoiceFieldsForState(stateFor("2024","monk",20,"open-hand")),paladin=classChoiceFieldsForState(stateFor("2024","paladin",20,"oath-devotion")),cleric=classChoiceFieldsForState(stateFor("2024","cleric",20,"life-domain"));assert.ok(keys(monk).includes("classSkills"));assert.ok(keys(monk).includes("monkTool"));assert.ok(keys(paladin).includes("classSkills"));assert.ok(keys(paladin).includes("fightingStyle"));for(const key of ["classSkills","divineOrder","blessedStrikes"])assert.ok(keys(cleric).includes(key));for(const fields of [monk,paladin,cleric])assert.ok(advancementFields(fields).length>0);
});
...
test("Class Options keeps unavailable choices out at low levels while retaining base class skills",()=>{
  ...
  assert.deepEqual(keys(classChoiceFieldsForState(stateFor("2024","warlock",1,"fiend-patron"))),["classSkills","eldritchInvocations"]);
