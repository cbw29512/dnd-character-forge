...
test("2014 Wizard and 2024 Cleric remain fail-closed above their verified level 5",()=>{
  for(const [ruleset,classId] of [["2014","wizard"],["2024","cleric"]]){const state=createInitialState();state.ruleset=ruleset;state.constraints.level="6";state.constraints.class=classId;assert.throws(()=>generateCharacter(state),/currently supports levels/i);}
});
