...
test("2024 Druid print preserves exact known forms and revised Wild Shape state",()=>{
  const character=druid("2024"),model=buildPremiumPrintModel(character),turn=buildQuickTurn(character).join(" "),cantrips=character.spells.cantrips.all,prepared=character.spells.prepared.all,always=character.spells.alwaysPrepared;
  assert.equal(model.profile.caster,true);assert.equal(model.packet.totalPages,2);assert.equal(model.theme.id,"druid-wild");assert.equal(model.classUtility.title,"The Old Wild");assert.equal(model.druidSupport.ruleset,"2024");assert.equal(model.druidSupport.fieldFormsAreExamples,false);assert.equal(model.druidSupport.forms.length,8);assert.ok(model.druidSupport.forms.some(form=>form.id==="pteranodon"));assert.equal(model.druidSupport.forms.some(form=>form.id==="giant-eagle"),false);assert.equal(model.druidSupport.wildShapeUses,4);assert.equal(model.druidSupport.wildShapeTempHp,20);assert.equal(cantrips.length,5);assert.equal(prepared.length,22);assert.equal(always.length,7);
  assert.deepEqual(intersection(cantrips,always),[],`cantrip/always overlap: ${intersection(cantrips,always).join(", ")}`);assert.deepEqual(intersection(prepared,always),[],`prepared/always overlap: ${intersection(prepared,always).join(", ")}`);assert.deepEqual(intersection(cantrips,prepared),[],`cantrip/prepared overlap: ${intersection(cantrips,prepared).join(", ")}`);assert.equal(new Set([...cantrips,...prepared,...always]).size,34,`unique Druid spell ids: ${[...new Set([...cantrips,...prepared,...always])].join(", ")}`);
  assert.equal(model.spellPage.entries.length,34);assert.ok(model.spellPage.entries.some(entry=>entry.id==="speak-with-animals"&&entry.tags.includes("A")));assert.ok(model.spellPage.entries.some(entry=>entry.id==="shocking-grasp"&&entry.tags.includes("A")));assert.match(turn,/8 known Wild Shape forms/i);assert.match(turn,/20 Temporary HP/i);assert.match(turn,/Wild Resurgence/i);
});
