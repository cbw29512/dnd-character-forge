...
test("2024 Druid print preserves exact known forms and revised Wild Shape state",()=>{
  const character=druid("2024"),preCantrips=[...character.spells.cantrips.all],prePrepared=[...character.spells.prepared.all],preAlways=[...character.spells.alwaysPrepared];
  assert.deepEqual(intersection(preCantrips,preAlways),[],`pre-export cantrip/always overlap: ${intersection(preCantrips,preAlways).join(", ")}`);assert.deepEqual(intersection(prePrepared,preAlways),[],`pre-export prepared/always overlap: ${intersection(prePrepared,preAlways).join(", ")}`);assert.deepEqual(intersection(preCantrips,prePrepared),[],`pre-export cantrip/prepared overlap: ${intersection(preCantrips,prePrepared).join(", ")}`);assert.equal(new Set([...preCantrips,...prePrepared,...preAlways]).size,34,`pre-export unique Druid spell ids: ${[...new Set([...preCantrips,...prePrepared,...preAlways])].join(", ")}`);
  const model=buildPremiumPrintModel(character),turn=buildQuickTurn(character).join(" ");
  assert.deepEqual(character.spells.cantrips.all,preCantrips,"print/reference construction mutated Druid cantrips");assert.deepEqual(character.spells.prepared.all,prePrepared,"print/reference construction mutated Druid prepared spells");assert.deepEqual(character.spells.alwaysPrepared,preAlways,"print/reference construction mutated Druid always-prepared spells");
  assert.equal(model.profile.caster,true);assert.equal(model.packet.totalPages,2);assert.equal(model.theme.id,"druid-wild");assert.equal(model.classUtility.title,"The Old Wild");assert.equal(model.druidSupport.ruleset,"2024");assert.equal(model.druidSupport.fieldFormsAreExamples,false);assert.equal(model.druidSupport.forms.length,8);assert.ok(model.druidSupport.forms.some(form=>form.id==="pteranodon"));assert.equal(model.druidSupport.forms.some(form=>form.id==="giant-eagle"),false);assert.equal(model.druidSupport.wildShapeUses,4);assert.equal(model.druidSupport.wildShapeTempHp,20);assert.equal(preCantrips.length,5);assert.equal(prePrepared.length,22);assert.equal(preAlways.length,7);assert.equal(model.spellPage.entries.length,34);assert.ok(model.spellPage.entries.some(entry=>entry.id==="speak-with-animals"&&entry.tags.includes("A")));assert.ok(model.spellPage.entries.some(entry=>entry.id==="shocking-grasp"&&entry.tags.includes("A")));assert.match(turn,/8 known Wild Shape forms/i);assert.match(turn,/20 Temporary HP/i);assert.match(turn,/Wild Resurgence/i);
});
