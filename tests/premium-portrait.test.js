...
test("portrait workflow is local, bounded, persistent, presentation-only, and mobile-print safe",()=>{
  const portrait=fs.readFileSync(new URL("../src/ui/portrait-upload.js",import.meta.url),"utf8"),app=fs.readFileSync(new URL("../src/app.js",import.meta.url),"utf8"),state=fs.readFileSync(new URL("../src/state.js",import.meta.url),"utf8"),css=fs.readFileSync(new URL("../styles/portrait.css",import.meta.url),"utf8"),print=fs.readFileSync(new URL("../src/ui/print.js",import.meta.url),"utf8");assert.match(portrait,/MAX_SOURCE_BYTES=8\*1024\*1024/);assert.match(portrait,/MAX_SIDE=560/);assert.match(portrait,/MAX_OUTPUT_CHARS=450000/);assert.match(portrait,/canvas\.toDataURL\("image\/jpeg"/);assert.match(portrait,/image\/webp/);assert.match(app,/bindPortraitUpload/);assert.match(app,/restorePortraitFromCharacter/);assert.match(app,/applyPortraitToCurrent/);assert.match(app,/exportCharacterPdf\(state\.currentCharacter\)/);assert.doesNotMatch(app,/await exportCharacterPdf/);assert.match(state,/portraitDataUrl: null/);assert.match(css,/ps-portrait-art\.has-image img/);assert.match(print,/primePrintImages\(root\)/);assert.match(print,/image\.decode/);assert.match(print,/window\.print\(\)/);assert.doesNotMatch(print,/await waitForPrintImages\(root\)/);
});
