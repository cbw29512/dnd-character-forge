...
// Data schema: every edition supplies its own class catalog. The stress matrix
// fixes only class + representative level while leaving species, background,
// subclass, equipment, spells, and feature choices on the normal Random path.
...
function stateFor({ruleset,classId="random",level="random"}){
  try{
    ...
    console.error(`[repeatability-stress] state build failed: ${ruleset} ${classId} L${level}`,error);
    throw error;
  }
}
...
function assertProductionCharacter(character,label){
  try{
    ...
    console.error(`[repeatability-stress] production assertion failed: ${label}`,error);
    throw error;
  }
}
...
for(const {ruleset,data} of EDITIONS){
  test(`${ruleset} repeated Random generation stays valid across every class at low/mid/high levels`,()=>{
    try{
      ...
      console.error(`[repeatability-stress] ${ruleset} class matrix failed`,error);
      throw error;
    }
  });
