...
import { MAX_PREGEN_BACKUP_BYTES, MAX_PREGEN_BACKUP_ENTRIES, PREGEN_BACKUP_FORMAT, PREGEN_BACKUP_VERSION, exportPregenBackup, importPregenBackup } from "../src/library/pregen-backup.js";
...
test("Pregen backup import rejects oversized payloads before JSON parsing",async()=>{
  storage.clear();
  const oversized="x".repeat(MAX_PREGEN_BACKUP_BYTES+1);
  await assert.rejects(()=>importPregenBackup(oversized),/5 MB or smaller/i);
  assert.equal(loadPregens().length,0);
});
...
test("Pregen backup controls are static, keyboard-reachable UI with mobile-safe target sizing",()=>{
  ...
  const ui=fs.readFileSync(new URL("../src/ui/library.js",import.meta.url),"utf8");
  ...
  assert.match(ui,/file\.size>MAX_PREGEN_BACKUP_BYTES/);
  assert.ok(ui.indexOf("file.size>MAX_PREGEN_BACKUP_BYTES")<ui.indexOf("await file.text()"),"file byte limit must be checked before reading the backup into memory");
});
