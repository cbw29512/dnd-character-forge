...
test("2014 Circle of the Land SRD slice contains seven complete land tables",()=>{
  const lands=["arctic","coast","desert","forest","grassland","mountain","swamp"];assert.equal(DRUID_CIRCLE_SPELLS_2014.length,56);assert.deepEqual([...new Set(DRUID_CIRCLE_SPELLS_2014.map(s=>s.land))].sort(),lands.sort());
