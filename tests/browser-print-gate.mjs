...
const CASES=[
  {ruleset:"2024",classId:"fighter",subclass:"champion",species:"human",background:"criminal",pages:1},
  {ruleset:"2024",classId:"rogue",subclass:"thief",species:"human",background:"criminal",pages:1},
  {ruleset:"2024",classId:"wizard",subclass:"evoker",species:"human",background:"criminal",pages:2},
  {ruleset:"2024",classId:"cleric",subclass:"life-domain",species:"human",background:"criminal",pages:2},
...
function verifyPacket(testCase){
  const character=characterAt(testCase),target={innerHTML:""},model=renderPremiumPrintSheet(character,target),slug=`${testCase.ruleset}-${testCase.species}-${testCase.classId}`,htmlPath=path.join(OUT,`${slug}.html`),pdfPath=path.join(OUT,`${slug}.pdf`),txtPath=path.join(OUT,`${slug}.txt`);
  ...
  execFileSync("pdftotext",["-layout",pdfPath,txtPath]);const extracted=readFileSync(txtPath,"utf8"),pageText=pdfPages(extracted);assert.equal(pageText.length,pages,`${slug}: extracted page count mismatch`);for(let index=0;index<pageText.length;index++){const text=normalize(pageText[index]);assert.ok(text.length>500,`${slug}: page ${index+1} is suspiciously sparse (${text.length} chars)`);assert.match(text,new RegExp(`Page\\s+${index+1}\\s*\\/\\s*${pages}`,"i"),`${slug}: page marker missing`);}
  const whole=normalize(extracted),tokens=[character.name,...model.ruleIndex.map(item=>item.name),...(model.spellPage?.entries||[]).map(item=>item.name)];for(const token of tokens)assert.ok(whole.includes(normalize(token)),`${slug}: printed PDF lost expected content: ${token}`);assert.ok(whole.includes("RAW integrity"),`${slug}: audit integrity marker missing`);
  legacyChecks(testCase,character,whole,model);console.log(`[browser-print] ${slug}: ${pages} Letter page${pages===1?"":"s"} · ${model.ruleIndex.length} rules · ${model.spellPage?.entries.length||0} spells`);
}
