...
function verifyPacket(testCase){
  ...
  const whole=normalize(extracted),tokens=[character.name,...model.attacks.map(item=>item.name),...model.equipment,...model.ruleIndex.map(item=>item.name),...(model.spellPage?.entries||[]).map(item=>item.name)];for(const token of tokens)assert.ok(whole.includes(normalize(token)),`${slug}: printed PDF lost expected content: ${token}`);assert.ok(whole.toLowerCase().includes("raw integrity"),`${slug}: audit integrity marker missing`);
  legacyChecks(testCase,character,whole,model);console.log(`[browser-print] ${slug}: ${pages} Letter page${pages===1?"":"s"} · ${model.ruleIndex.length} rules · ${model.equipment.length} equipment · ${model.spellPage?.entries.length||0} spells`);
}
