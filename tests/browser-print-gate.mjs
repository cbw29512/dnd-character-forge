...
const CASES=[
  ...
  {ruleset:"2014",classId:"cleric",subclass:"life-domain",species:"dwarf",background:"acolyte",speciesSelections:{tool:"masons-tools"}},
...
function verifyPacket(testCase){
  const character=characterAt(testCase),target={innerHTML:""},model=renderPremiumPrintSheet(character,target),slug=`${testCase.ruleset}-${testCase.species}-${testCase.classId}-${testCase.subclass}`,htmlPath=path.join(OUT,`${slug}.html`),pdfPath=path.join(OUT,`${slug}.pdf`),txtPath=path.join(OUT,`${slug}.txt`);
  ...
  if(testCase.ruleset==="2014"&&testCase.classId==="cleric"){assert.equal(character.spells.cantrips.all.length,5);assert.ok(character.spells.prepared.all.length>=24);assert.equal(character.spells.alwaysPrepared.length,10);assert.ok(spells.length>=39);assert.ok(whole.includes("SRD 5.1"));assert.ok(whole.includes("Cleric spell list pp.106-107"));assert.ok(whole.includes("Destroy Undead (CR 4)"));assert.ok(whole.includes("Divine Intervention"));assert.ok(whole.includes("Supreme Healing"));}
  if(testCase.ruleset==="2014"&&testCase.species==="dwarf"){assert.equal(character.speciesChoices.subraceName,"Hill Dwarf");assert.equal(character.speciesChoices.toolName,"Mason's Tools");assert.equal(character.speciesHpBonus,20);assert.ok(character.languages.includes("Common"));assert.ok(character.languages.includes("Dwarvish"));assert.ok(whole.includes("Hill Dwarf"));assert.ok(whole.includes("Dwarven Toughness"));assert.ok(whole.includes("Mason's Tools"));}
...
function characterAt({ruleset,classId,subclass,species,background,speciesSelections={}}){const state=createInitialState();state.ruleset=ruleset;state.constraints.level="20";state.constraints.class=classId;state.constraints.subclass=subclass;state.constraints.species=species;state.constraints.background=background;state.speciesSelections=speciesSelections;return generateCharacter(state);}
