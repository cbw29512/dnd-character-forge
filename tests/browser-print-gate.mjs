...
function verifyPacket(testCase){
  ...
  const whole=normalize(extracted),tokens=[character.name,...model.attacks.map(item=>item.name),...model.equipment,...model.ruleIndex.map(item=>item.name),...(model.classUtility?[model.classUtility.title]:[]),...(model.spellPage?.entries||[]).map(item=>item.name)];for(const token of tokens)assert.ok(whole.includes(normalize(token)),`${slug}: printed PDF lost expected content: ${token}`);assert.ok(whole.toLowerCase().includes("raw integrity"),`${slug}: audit integrity marker missing`);
