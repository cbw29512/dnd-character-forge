...
test("Barbarian picker catalog exposes Berserker plus two clearly labeled Forge originals in each edition",()=>{
  for(const [ruleset,data] of [["2014",FORGE_2014],["2024",FORGE_2024]]){
    ...
    for(const expected of ORIGINALS){const subclass=subclasses.find(item=>item.id===expected.id);assert.ok(subclass,`${ruleset} missing ${expected.id}`);assert.equal(subclass.contentKind,"forge-original");assert.match(subclass.displayName,/Forge Original/);}
  }
});
...
test("Forge-original Barbarian feature unlocks are exact at levels 3, 6, 10, and 14 in both editions",()=>{
  for(const ruleset of ["2014","2024"])for(const subclass of ORIGINALS)for(const [level,name] of subclass.features){
    const at=barbarian(ruleset,subclass.id,level),before=level===3?barbarian(ruleset,"berserker",2):barbarian(ruleset,subclass.id,level-1);
    assert.equal(at.validation.valid,true,`${ruleset} ${subclass.id} L${level} validation`);assert.ok(at.features.includes(name),`${ruleset} ${subclass.id} missing ${name} at L${level}`);if(level>3)assert.equal(before.features.includes(name),false,`${ruleset} ${subclass.id} gained ${name} early`);
