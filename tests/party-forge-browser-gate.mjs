...
async function verify(item,port){
  ...
  if(item.width<=390){assert.ok(audit.scrollY>100,`${item.name}: Party Forge did not move the generated roster into view`);assert.ok(audit.rosterTop>=-2&&audit.rosterTop<item.height*.4,`${item.name}: generated roster is not near the visible viewport after Forge (${audit.rosterTop}px)`);}
