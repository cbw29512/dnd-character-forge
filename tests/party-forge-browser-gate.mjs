...
async function verify(item,port){
  ...
  console.log(`[party-forge-browser] ${item.name}: ${audit.memberCount} members · overflow ${audit.horizontalOverflow}px`);
}

function auditScript(){return `<script>(()=>{let auditError="";window.addEventListener("error",event=>{auditError=event?.error?.message||event?.message||"window error";});window.addEventListener("unhandledrejection",event=>{auditError=event?.reason?.message||String(event?.reason||"unhandled rejection");});const run=()=>setTimeout(()=>{try{const params=new URLSearchParams(location.search),ruleset=params.get("ruleset")||"2024",rules=document.getElementById("ruleset");rules.value=ruleset;rules.dispatchEvent(new Event("change",{bubbles:true}));const toggle=document.getElementById("partyForgeToggle");toggle?.click();const size=document.getElementById("partySize"),level=document.getElementById("partyLevel"),composition=document.getElementById("partyComposition");if(size)size.value="4";if(level)level.value="5";if(composition)composition.value="balanced";document.getElementById("forgePartyButton")?.click();setTimeout(()=>{const root=document.documentElement,body=document.body,panel=document.getElementById("partyForgePanel"),proof=document.querySelector(".party-roster-proof"),data={auditError,togglePresent:Boolean(toggle),panelVisible:Boolean(panel&&!panel.hidden),rosterPresent:Boolean(document.querySelector(".party-roster")),memberCount:document.querySelectorAll(".party-member-card").length,validatedProof:Boolean(proof&&/4\\/4 RAW validated/.test(proof.textContent)),ruleset:document.getElementById("ruleset")?.value||"",horizontalOverflow:Math.max(0,Math.max(root.scrollWidth,body.scrollWidth)-root.clientWidth)};const result=document.createElement("pre");result.id="partyAuditResult";result.textContent=JSON.stringify(data);document.body.append(result);},900);}catch(error){auditError=error.message;const result=document.createElement("pre");result.id="partyAuditResult";result.textContent=JSON.stringify({auditError,togglePresent:false,panelVisible:false,rosterPresent:false,memberCount:0,validatedProof:false,ruleset:"",horizontalOverflow:999});document.body.append(result);}},200);if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",run,{once:true});else run();})();<\/script>`;}
