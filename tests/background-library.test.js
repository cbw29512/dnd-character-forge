...
function characterAt(ruleset,background,{subclass="champion"}={}){
  const state=createInitialState();state.sourceMode=SOURCE.HOMEBREW;state.ruleset=ruleset;state.constraints.level="5";state.constraints.species=ruleset==="2014"?"human":"dwarf";state.constraints.class="fighter";state.constraints.subclass=subclass;state.constraints.background=background;state.constraints.name=`Background ${background}`;return generateCharacter(state);
}
...
test("compatible background library stays explicitly source-labeled outside RAW mode",()=>{
...
test("every 2014 original background remains testable only as explicit compatible content",()=>{
  for(const background of FORGE_ORIGINAL_BACKGROUNDS_2014){
    ...
    assert.equal(character.sourceMode,SOURCE.HOMEBREW);assert.equal(character.validation.valid,true,background.id);assert.equal(character.audit.status,"PASS",background.id);assert.equal(character.audit.rawIntegrity,false,background.id);assert.match(character.audit.license,/Character Forge Original/);assert.match(character.audit.scope,/official non-SRD D&D material is not reproduced/i);
...
test("every 2024 original background remains testable only as explicit compatible content",()=>{
  for(const background of FORGE_ORIGINAL_BACKGROUNDS_2024){
    ...
    assert.equal(character.sourceMode,SOURCE.HOMEBREW);assert.equal(character.validation.valid,true,background.id);assert.equal(character.audit.status,"PASS",background.id);assert.equal(character.audit.rawIntegrity,false,background.id);assert.match(character.audit.license,/Character Forge Original/);
    for(const skill of background.skills)assert.ok(character.skills.includes(skill),`${background.id}: ${skill}`);
...
test("Forge Original background records remain opt-in and cannot override Random eligibility",()=>{
  for(const [ruleset,data,sources] of [["2014",FORGE_2014,FORGE_ORIGINAL_BACKGROUNDS_2014],["2024",FORGE_2024,FORGE_ORIGINAL_BACKGROUNDS_2024]]){
    const originals=data.backgrounds.filter(isForgeOriginalBackground);
    assert.equal(originals.length,sources.length,`${ruleset}: original catalog count`);
    assert.ok(originals.every(item=>item.randomEligible===false),`${ruleset}: original Random exclusion missing`);
    assert.ok(sources.every(item=>item.randomEligible===false),`${ruleset}: source records were mutated`);
  }
});

test("original background and original subclass can coexist only in explicit compatible mode",()=>{
