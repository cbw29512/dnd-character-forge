...
try{
  ...
  console.log(`[curated-dossier] ${SLUG}: SVG decoded in Chrome and dossier page rendered to ${path.basename(pngPath)}`);
}catch(error){
  console.error("[curated-dossier] browser/PDF certification failed",error);
  throw error;
}
...
function buildCharacter(){
  try{
    ...
    assert.equal(character.validation.valid,true,"curated pilot fixture must remain legal");
    character.presentation={...(character.presentation||{}),sheetCustomization:{packetMode:"deluxe",printMode:"premium"}};
    return character;
  }catch(error){
    console.error("[curated-dossier] fixture build failed",error);
    throw error;
  }
}
...
function fixtureHtml(packet){
  try{
    ...
    console.error("[curated-dossier] fixture HTML failed",error);
    throw error;
  }
}
