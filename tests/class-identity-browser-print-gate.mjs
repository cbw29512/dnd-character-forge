...
function verifyClass(classId){
  ...
  const dossierText=normalize(execFileSync("pdftotext",["-f",String(expected),"-l",String(expected),"-raw",pdfPath,"-"],{encoding:"utf8"})),dossierKey=storyKey(dossierText);assert.ok(dossierKey.includes(storyKey(model.dossier.storyTitle)),`${classId}: dossier story title clipped or missing`);for(const [index,paragraph] of model.dossier.backstory.entries())assert.ok(dossierKey.includes(storyKey(paragraph)),`${classId}: dossier backstory paragraph ${index+1} clipped or missing`);assert.match(dossierText,/Campaign Chronicle/i,`${classId}: Campaign Chronicle clipped or missing`);assert.match(dossierText,/Milestones.*Allies.*Debts.*Revelations.*Session Notes/i,`${classId}: Chronicle guidance clipped or missing`);
...
function characterAt(classId){
  const state=createInitialState();state.ruleset="2024";state.constraints.level="7";state.constraints.class=classId;state.constraints.subclass="random";state.constraints.species="human";state.constraints.background="criminal";const character=withSeed(seedFor(classId),()=>generateCharacter(state));character.presentation={...(character.presentation||{}),sheetCustomization:{packetMode:"deluxe",style:"ornate",paper:"ivory",ornament:"rich",frame:"class",printMode:"premium",portraitVisible:true}};return character;
}
...
function storyKey(value){return String(value||"").normalize("NFKD").toLowerCase().replace(/[^\p{L}\p{N}]+/gu,"");}
