...
console.log(`[class-identity-browser-print] verified ${CLASSES.length} deluxe class identities in Chrome`);
...
function verifyClass(classId){
  try{
    ...
    execFileSync("pdftotext",["-layout",pdfPath,txtPath]);const text=normalize(readFileSync(txtPath,"utf8"));assert.ok(text.includes(character.name),`${classId}: name missing`);assert.ok(text.includes(character.class.name),`${classId}: class missing`);assert.ok(text.includes("Deluxe Character Dossier"),`${classId}: dossier heading missing`);assert.ok(text.includes("Generated narrative flavor"),`${classId}: narrative disclaimer missing`);assert.ok(text.toLowerCase().includes("raw integrity"),`${classId}: RAW marker missing`);assert.match(text,new RegExp(`Page ${expected}\\s*\\/\\s*${expected}`,"i"),`${classId}: final page marker missing`);
    execFileSync("pdftoppm",["-png","-r","96","-f","1","-singlefile",pdfPath,sheetPng]);execFileSync("pdftoppm",["-png","-r","96","-f",String(expected),"-singlefile",pdfPath,dossierPng]);
    console.log(`[class-identity-browser-print] ${classId}: ${expected} pages · ${model.theme.id}`);
