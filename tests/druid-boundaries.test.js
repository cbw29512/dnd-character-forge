...
test("2014 Druid Wild Shape boundaries preserve seen-beast semantics and movement gates",()=>{
  const one=druid("2014",1);assert.equal(one.druid.wildShapeUses,0);assert.deepEqual(one.druidSelections.fieldForms,[]);assert.ok(one.languages.includes("Druidic"));assert.ok(one.toolProficiencies.includes("Herbalism Kit"));
  const two=druid("2014",2,{subclass:"circle-land",classSelections:{circleLand:"forest",fieldForms:["wolf"]}});assert.equal(two.druid.wildShapeUses,2);assert.equal(two.druid.maxCr,.25);assert.equal(two.druid.allowSwim,false);assert.equal(two.druid.allowFly,false);assert.equal(two.druidSelections.fieldForms.length,4);assert.equal(two.druidSelections.fieldFormsAreExamples,true);assert.ok(two.druidSelections.fieldForms.includes("wolf"));
  ...
  const twenty=druid("2014",20,{subclass:"circle-land",classSelections:{circleLand:"forest",fieldForms:["giant-eagle","brown-bear","giant-spider","reef-shark"]}});assert.equal(twenty.druid.unlimitedWildShape,true);assert.equal(twenty.spells.cantrips.all.length,5);assert.equal(twenty.spells.prepared.all.length,20+Math.floor((twenty.abilities.wis-10)/2));assert.equal(twenty.spells.alwaysPrepared.length,8);assert.equal(twenty.audit.status,"PASS");
});

test("2024 Druid known-form counts and flight gates are exact",()=>{
  const one=druid("2024",1,{classSelections:{primalOrder:"magician"}});assert.equal(one.druid.knownFormCount,0);assert.deepEqual(one.druidSelections.knownForms,[]);assert.equal(one.spells.cantrips.all.length,3);assert.ok(one.spells.alwaysPrepared.includes("speak-with-animals"));assert.ok(one.languages.includes("Druidic"));
  const two=druid("2024",2,{classSelections:{primalOrder:"warden",knownForms:["rat","riding-horse","spider","wolf"]}});assert.equal(two.druidSelections.knownForms.length,4);assert.equal(two.druid.allowFly,false);for(const id of two.druidSelections.knownForms)assert.equal(Boolean(druidFormById("2024",id).speeds.fly),false);
  const four=druid("2024",4,{subclass:"circle-land",classSelections:{primalOrder:"magician",circleLand:"temperate"}});assert.equal(four.druidSelections.knownForms.length,6);assert.equal(four.druid.maxCr,.5);assert.equal(four.druid.allowFly,false);
  ...
  const eight=druid("2024",8,{subclass:"circle-land",classSelections:{primalOrder:"magician",circleLand:"temperate",elementalFury:"potent-spellcasting",knownForms:["pteranodon"]}});assert.equal(eight.druidSelections.knownForms.length,8);assert.ok(eight.druidSelections.knownForms.includes("pteranodon"));assert.equal(eight.druid.allowFly,true);
