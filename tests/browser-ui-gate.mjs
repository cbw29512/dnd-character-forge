...
async function verifyViewport(item,port){
  ...
  const png=path.join(OUT,`${item.name}.png`);await execFileAsync(CHROME,[...common,`--screenshot=${png}`,url],{encoding:"utf8",timeout:30000,maxBuffer:4*1024*1024});console.log(`[browser-ui] ${item.name}: ${audit.viewportWidth}px · overflow ${audit.horizontalOverflow}px · character rendered`);
}
function auditScript(){return `<script>window.addEventListener("load",()=>setTimeout(()=>{const forge=document.getElementById("forgeButton");if(forge)forge.click();setTimeout(()=>{const visible=element=>{if(!element)return false;const style=getComputedStyle(element),rect=element.getBoundingClientRect();return style.display!=="none"&&style.visibility!=="hidden"&&rect.width>0&&rect.height>0;};const box=selector=>{const element=document.querySelector(selector);if(!element)return {present:false,visible:false};const rect=element.getBoundingClientRect();return {present:true,visible:visible(element),left:Math.round(rect.left*100)/100,right:Math.round(rect.right*100)/100,width:Math.round(rect.width*100)/100,height:Math.round(rect.height*100)/100};};const root=document.documentElement,body=document.body,error=document.getElementById("error"),forgeButton=document.getElementById("forgeButton");const data={viewportWidth:root.clientWidth,horizontalOverflow:Math.max(0,Math.max(root.scrollWidth,body.scrollWidth)-root.clientWidth),characterRendered:Boolean(document.querySelector("#result .character-sheet")),errorVisible:visible(error),errorText:error?.textContent?.trim()||"",forgeButton:{visible:visible(forgeButton),height:forgeButton?forgeButton.getBoundingClientRect().height:0},boxes:{".site-header":box(".site-header"),".forge-panel":box(".forge-panel"),".result-stage":box(".result-stage"),".character-sheet":box("#result .character-sheet"),".forge-button":box("#forgeButton"),".primary-nav":box(".primary-nav")}};const result=document.createElement("pre");result.id="uiAuditResult";result.textContent=JSON.stringify(data);document.body.append(result);},500);},100));<\/script>`;}
