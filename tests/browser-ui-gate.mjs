...
async function verifyViewport(item,port){
  ...
  if(item.width<=390){const forgePanel=audit.boxes[".forge-panel"],limit=item.height*.82;assert.ok(audit.initialForgePanelTop<=limit,`${item.name}: landing Forge controls begin too far below the first viewport (${audit.initialForgePanelTop}px > ${Math.round(limit)}px)`);assert.equal(forgePanel.present,true,`${item.name}: Forge panel is missing`);assert.equal(forgePanel.visible,true,`${item.name}: Forge panel is hidden`);assert.ok(forgePanel.top<=limit,`${item.name}: generated-workspace Forge controls begin too far below the first viewport (${forgePanel.top}px > ${Math.round(limit)}px)`);}
  console.log(`[browser-ui] ${item.name}: ${audit.viewportWidth}px · overflow ${audit.horizontalOverflow}px · landing Forge y=${audit.initialForgePanelTop}px · generated Forge y=${audit.boxes[".forge-panel"].top}px · character rendered`);
}
