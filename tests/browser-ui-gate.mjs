...
async function verifyViewport(item,port){
  ...
  }catch(error){console.error(`[browser-ui] ${item.name}: viewport verification failed`,error);throw error;}
}
...
async function dumpAuditDom(item,url,common){
  ...
  for(let attempt=1;attempt<=2;attempt+=1){
    try{
      const result=await execFileAsync(CHROME,[...common,"--dump-dom",url],{encoding:"utf8",timeout:30000,maxBuffer:8*1024*1024});
      lastDom=result.stdout||"";lastStderr=result.stderr||"";
      if(/<pre id="uiAuditResult">[^<]+<\/pre>/.test(lastDom))return lastDom;
      console.warn(`[browser-ui] ${item.name}: audit marker missing on Chrome dump attempt ${attempt}/2; retrying with a fresh browser process`);
    }catch(error){
      console.error(`[browser-ui] ${item.name}: Chrome dump attempt ${attempt}/2 failed`,error);if(attempt===2)throw error;
    }
  }
  const tail=lastDom.slice(-1200).replace(/\s+/g," ").trim(),stderr=lastStderr.slice(-800).replace(/\s+/g," ").trim();
  throw new Error(`${item.name}: UI audit result was not produced after 2 Chrome attempts.${tail?` DOM tail: ${tail}`:""}${stderr?` Chrome stderr: ${stderr}`:""}`);
}
