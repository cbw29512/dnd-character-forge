...
const server=createServer((req,res)=>{
  try{
    ...
    if(url.pathname==="/__ui-audit.html"){
...
async function verifyViewport(item,port){
  ...
  assert.equal(audit.characterRendered,true,`${item.name}: Forge did not render a character`);
  assert.equal(audit.errorVisible,false,`${item.name}: Forge displayed an error: ${audit.errorText||"unknown"}`);
  assert.ok(audit.horizontalOverflow<=1,`${item.name}: page has ${audit.horizontalOverflow}px horizontal overflow`);
  assert.equal(audit.forgeButton.visible,true,`${item.name}: Forge Character action is hidden`);
  assert.ok(audit.forgeButton.height>=40,`${item.name}: Forge Character action is too short for touch/keyboard use`);
  for(const [selector,box] of Object.entries(audit.boxes)){
    if(!box.present||!box.visible)continue;
    assert.ok(box.left>=-1,`${item.name}: ${selector} escapes left viewport (${box.left})`);
    assert.ok(box.right<=audit.viewportWidth+1,`${item.name}: ${selector} escapes right viewport (${box.right} > ${audit.viewportWidth})`);
...
function auditScript(){
  ...
  window.addEventListener("load",()=>setTimeout(()=>{
    ...
    setTimeout(()=>{
      ...
      const box=selector=>{const element=document.querySelector(selector);if(!element)return {present:false,visible:false};const rect=element.getBoundingClientRect();return {present:true,visible:visible(element),left:Math.round(rect.left*100)/100,right:Math.round(rect.right*100)/100,width:Math.round(rect.width*100)/100,height:Math.round(rect.height*100)/100};};
      const root=document.documentElement,body=document.body,error=document.getElementById("error"),forgeButton=document.getElementById("forgeButton");
      const data={
        viewportWidth:root.clientWidth,
        horizontalOverflow:Math.max(0,Math.max(root.scrollWidth,body.scrollWidth)-root.clientWidth),
        characterRendered:Boolean(document.querySelector("#result .character-sheet")),
        errorVisible:visible(error),
        errorText:error?.textContent?.trim()||"",
        forgeButton:{visible:visible(forgeButton),height:forgeButton?forgeButton.getBoundingClientRect().height:0},
        boxes:{
          ".site-header":box(".site-header"),
          ".forge-panel":box(".forge-panel"),
          ".result-stage":box(".result-stage"),
          ".character-sheet":box("#result .character-sheet"),
          ".forge-button":box("#forgeButton"),
          ".primary-nav":box(".primary-nav")
        }
      };
      const result=document.createElement("pre");result.id="uiAuditResult";result.textContent=JSON.stringify(data);document.body.append(result);
    },500);
  },100));
  <\/script>`;
}
