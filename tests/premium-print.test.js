...
test("casters use a hard two-page export profile",()=>{
  for(const [classId,subclass] of [["wizard","evoker"],["cleric","life-domain"]]){const m=buildPremiumPrintModel(characterAt(classId,20,subclass));assert.equal(m.profile.id,"caster-two-page");assert.equal(m.packet.totalPages,2);assert.ok(m.spellPage);}
});
...
test("compact rules index preserves every active sourced rule",()=>{
  for(const [classId,subclass] of [["fighter","champion"],["wizard","evoker"],["cleric","life-domain"],["rogue","thief"]]){const c=characterAt(classId,20,subclass),m=buildPremiumPrintModel(c),expected=buildQuickReference(c).map(item=>item.id).sort(),actual=m.ruleIndex.map(item=>item.id).sort();assert.deepEqual(actual,expected);assert.ok(m.ruleIndex.every(item=>/p\.\d+/.test(item.source)));}
});
test("caster page two preserves every generated spell exactly once",()=>{
  for(const [classId,subclass] of [["wizard","evoker"],["cleric","life-domain"]]){const c=characterAt(classId,20,subclass),m=buildPremiumPrintModel(c),actual=m.spellPage.entries.map(item=>item.id),expected=classId==="wizard"?[...new Set([...c.spells.cantrips.all,...c.spells.spellbook.all])]:[...new Set([...c.spells.cantrips.all,...c.spells.alwaysPrepared,...c.spells.prepared.all])];assert.equal(new Set(actual).size,actual.length);assert.deepEqual([...actual].sort(),[...expected].sort());}
});
...
test("martial renderer creates one sheet and caster renderer exactly two",()=>{
  ...
  renderPremiumPrintSheet(characterAt("wizard",20,"evoker"),target);assert.equal((target.innerHTML.match(/class="premium-sheet/g)||[]).length,2);assert.match(target.innerHTML,/Spell Loadout/);assert.match(target.innerHTML,/Sourced Rules Index/);assert.match(target.innerHTML,/Rules Audit/);
});
test("fixed export CSS and renderer enforce the new product contract",()=>{
  const renderer=readFileSync(new URL("../src/ui/premium-print.js",import.meta.url),"utf8"),fixed=readFileSync(new URL("../styles/print/premium-fixed.css",import.meta.url),"utf8"),responsive=readFileSync(new URL("../styles/responsive.css",import.meta.url),"utf8"),art=readFileSync(new URL("../src/print/class-art.js",import.meta.url),"utf8");assert.match(renderer,/renderPrintPageOne/);assert.match(renderer,/renderPrintPageTwo/);assert.match(fixed,/profile-martial-one-page/);assert.match(fixed,/profile-caster-two-page/);assert.match(fixed,/ps-spell-ledger/);assert.match(fixed,/theme-barbarian-rage/);assert.match(responsive,/print\/premium-fixed\.css/);assert.match(art,/barbarianArt/);
});
