...
test("martial renderer creates one sheet and caster renderer exactly two",()=>{
  ...
  renderPremiumPrintSheet(characterAt("wizard",20,"evoker"),target);assert.equal((target.innerHTML.match(/class="premium-sheet/g)||[]).length,2);assert.match(target.innerHTML,/>Spells</);assert.match(target.innerHTML,/Sourced Rules Index/);assert.match(target.innerHTML,/Rules Audit/);
});
