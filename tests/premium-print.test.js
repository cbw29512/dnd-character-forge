...
test("martial renderer creates one sheet and caster renderer exactly two",()=>{
  const target={innerHTML:""};renderPremiumPrintSheet(characterAt("fighter",20,"champion"),target);assert.equal((target.innerHTML.match(/class="premium-sheet/g)||[]).length,1);assert.match(target.innerHTML,/Active Rules/);assert.doesNotMatch(target.innerHTML,/Rules Index|Sourced Rules Index|Rules Audit/);
  renderPremiumPrintSheet(characterAt("paladin",20,"oath-devotion"),target);assert.equal((target.innerHTML.match(/class="premium-sheet/g)||[]).length,2);assert.match(target.innerHTML,/>Spells</);assert.match(target.innerHTML,/Active Rules/);assert.doesNotMatch(target.innerHTML,/Sourced Rules Index|Rules Audit/);
});
