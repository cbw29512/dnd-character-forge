...
async function verify(item,port){
  ...
  assert.equal(audit.summaryPresent,true,`${item.name}: DM Party Quick Reference is missing from print staging`);
  assert.equal(audit.summaryIsFirst,true,`${item.name}: DM Party Quick Reference must be the first printed page`);
  assert.equal(audit.summaryCardCount,4,`${item.name}: DM quick reference did not include four member cards`);
  assert.equal(audit.summaryNameCount,4,`${item.name}: DM quick reference omitted a party member name`);
  assert.equal(audit.summaryCertified,true,`${item.name}: DM quick reference lost the 4/4 certification proof`);
