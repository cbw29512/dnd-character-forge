...
console.log(`[class-signature-ink-saver] verified ${CASES.length} monochrome crisp class placeholders and rendered pages in Chrome`);
...
function verifyInkSaver(classId){
  try{
    ...
    assert.equal(character.validation.valid,true,`${classId}: generated character invalid`);assert.equal(character.audit.rawIntegrity,true,`${classId}: RAW integrity failed`);assert.equal(model.presentation.customization.printMode,"ink-saver",`${classId}: Ink Saver state missing`);assert.match(html,/sheet-print-ink-saver/,`${classId}: Ink Saver class missing`);assert.match(html,new RegExp(`data-print-class="${id}"`),`${classId}: print class marker missing`);assert.match(html,new RegExp(`<span>${escapeRegex(model.theme.className)}</span>`),`${classId}: visible class wordmark missing`);assert.match(html,/ps-class-signature/,`${classId}: class signature missing`);assert.match(html,/ps-class-ornaments/,`${classId}: class frame art missing`);
    assert.match(html,/ps-placeholder-svg ps-class-crest/,`${classId}: crisp vector placeholder missing`);assert.match(html,/shape-rendering="geometricPrecision"/,`${classId}: geometric precision hint missing`);assert.doesNotMatch(html,/ps-premium-class-crest|<filter\b|feDropShadow|linearGradient/,`${classId}: legacy filtered crest leaked into Ink Saver packet`);assert.match(html,new RegExp(`class-placeholder class-${id}[\\s\\S]*?ps-placeholder-svg`),`${classId}: first-page class placeholder missing`);assert.match(html,new RegExp(`ps-dossier-portrait-art[^\"]*\\bclass-${id}\\b[\\s\\S]*?ps-placeholder-svg`),`${classId}: dossier class placeholder missing`);assert.match(html,new RegExp(escapeRegex(model.theme.rail)),`${classId}: signature rail missing`);
    ...
    console.log(`[class-signature-ink-saver] ${classId}: ${expected} pages · ${model.theme.grayscalePattern} · crisp vector · rendered pixel monochrome`);
