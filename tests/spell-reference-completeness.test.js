...
function supported2024Spells(){
  ...
  for(const classId of CASTERS){
    ...
    for(const spell of spellDisplayRecords(character)){
      ...
      if(current){
        assert.equal(normalizeApostropheTypography(current.name),normalizeApostropheTypography(spell.name),`conflicting supported name for ${spell.id}`);
...
test("spell reference identity ignores only straight-vs-curly apostrophe typography",()=>{
  assert.equal(normalizeApostropheTypography("Dragon’s Breath"),normalizeApostropheTypography("Dragon's Breath"));
  assert.notEqual(normalizeApostropheTypography("Dragon Breath"),normalizeApostropheTypography("Dragon's Breath"));
});
...
test("2024 spell reference identity and level match the supported spell catalog",()=>{
  for(const spell of supported2024Spells()){
    ...
    assert.equal(normalizeApostropheTypography(reference.name),normalizeApostropheTypography(spell.name),`${spell.id}: reference name drift`);
