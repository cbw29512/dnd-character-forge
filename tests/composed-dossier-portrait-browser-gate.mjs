...
try{
  ...
  assert.match(target.innerHTML,/ps-composed-dossier-overlay/);
  ...
  assert.match(dom,/data-composed-loaded="true"/,"Chrome did not render the composed portrait layers");
  ...
  console.log(`[composed-dossier] ${SLUG}: reusable layers rendered in Chrome and PDF.`);
}catch(error){
...
function fixtureHtml(packet){
  try{
    return `<!doctype html><html><head><meta charset="utf-8"><link rel="stylesheet" href="../../styles/responsive.css"></head><body class="premium-print-active"><div id="premiumPrintRoot" class="premium-print-root">${packet}</div><script>window.addEventListener("load",async()=>{const portrait=document.querySelector("[data-composed-portrait]"),img=portrait?.querySelector("img"),overlay=portrait?.querySelector(".ps-composed-dossier-overlay");try{if(img)await img.decode();}catch(error){console.error(error);}document.body.dataset.composedLoaded=String(Boolean(img&&img.complete&&img.naturalWidth>0&&overlay));});</script></body></html>`;
